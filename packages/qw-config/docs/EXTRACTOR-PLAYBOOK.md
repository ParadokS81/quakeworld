...
**Extraction side (packages/qw-config):**
...
```
  packages/qw-config/src/data/*-ast.json  +  <engine>/help_*.json
        |
        v
...
Mirror `packages/qw-config/scripts/extractor_lib/clang_config.py`'s shape, but:
...
```bash
python3 packages/qw-config/scripts/extract-<engine>-unified.py \
  --repo-root research/repos/<engine>-source \
  --output-dir packages/qw-config/src/data \
...
npm --prefix apps/qw-oracle run load-knowledge -- load-version \
  ...
  --json packages/qw-config/src/data/<engine>-variables-ast.json \
