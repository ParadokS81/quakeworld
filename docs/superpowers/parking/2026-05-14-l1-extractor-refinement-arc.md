...
## RETRACTED (2026-05-15): "platform-conditional cvars are source-invisible" was a hallucinated finding

The section originally here (commit `291268bd`, 2026-05-15) claimed
`s_stereo` is "a real, live, Linux-only sound cvar" that the libclang
extractor "structurally never sees" because it "parses a fixed non-Linux
platform config," and generalized that platform-gated cvars are
systematically invisible to extraction. **Every load-bearing claim in that
section is false.** It was labelled "(verified)" but was not verified.
Refuted the same day by primary-source git + extractor-config inspection.

**Verified `s_stereo` lifecycle (ezquake-source `git log -S`):**
- 2005-06-03 `b8236019` -- added (ALSA/Linux sound, ported from ZQuake).
  It *was* a real Linux sound cvar, historically. That part of the
  intuition was correct.
- 2013-09-20 `0538d9ae` + `1215c7c3` -- removed (SDL2 sound backend
  import, legacy sound purge). 2015-11-03 `38845510` -- last dangling
  reference cleaned.
- ezquake **v3.0 (our earliest extracted tag) = 2016-06-04**, ~3 years
  *after* the purge. `git grep s_stereo` at v3.0 / 3.6.0 / 3.6.9 = 0;
  absent in `upstream/master`. It is not "live" -- it has been gone from
  source since before the extraction window begins. `source_file` NULL at
  all 10 tags is correct, not an extractor miss.
- The flicker is `help_variables.json` hand-curation: present 3.1,
  dropped 3.2.x, restored 3.6.0+ (verified by grepping the help file per
  tag) -- doc-file drift for code deleted years earlier, not the
  "build-target variance" the retracted section speculated.

**The extractor is NOT blind to Linux code.** It parses 4 translation
units per file (client / server / win / apple). The Linux libclang host
auto-predefines `__linux__` as the *baseline*; the win/apple variants
must explicitly `-U__linux__` to reach non-Linux paths (see
`apps/qw-oracle/scripts/extractors/extractor_lib/clang_config.py` and
`scripts/extractors/ezquake/extract.py`). A `#ifdef __linux__` cvar is in
the default parse. The "(non-Linux)" descriptor in the old pointer was
wrong; no parse-matrix widening is warranted -- its premise does not hold.

**Resolution (topic closed):**
- Issue A (the `source_retired` label): the intended output of the
  `3be4d576` entity-state-retreat fix under the codebase's temporal state
  model; functionally inert (snapshot excludes doc_only and
  source_retired identically; MCP only labels, never filters). No code
  change -- documented in
  `docs/superpowers/parking/2026-05-15-l1-extractor-entity-classification-followups.md`.
- Issue B (the F2 false flicker): fixed in commit `9a5a0c2d` --
  `F2.flickering_presence` now excludes pure help-JSON ghosts by the
  provenance fact (no `source_file` at any version) instead of the
  doc_only label that `3be4d576` invalidated. ezquake F2.flickering
  FOUND 1 -> CLEAN; fte/mvdsv/qwcl/ktx unchanged.
