...
**CRITICAL CROSS-BATCH GAP -- `k_fb_admin_only` MISSING from L1 catalog entirely** (verified 2026-05-26 by Frogbot batch dispatcher at handoff draft time): `grep -n 'data-name="k_fb_admin_only"' /home/paradoks/projects/quakeworld/apps/qw-oracle/docs/reviews/2026-05-22-ktx-l1-catalog.html` returns zero hits. The cvar is REGISTERED in source (`RegisterCvarEx(FB_CVAR_ADMIN_ONLY, "0")` at `src/world.c:1061`; constant defined as `"k_fb_admin_only"` at `include/fb_globals.h:405`) but was NOT extracted into the L1 entities table. This means: (a) all 78 Frogbot cards have `k_fb_admin_only` in See-also pointing at a non-existent L1 entity -- will be dangling references after apply pass; (b) this Admin & permissions batch CANNOT draft a v2 card for `k_fb_admin_only` because the pre-flight gate at the per-card skill aborts when the entity doesn't resolve to a live L1 entity (`Hard pre-flight gate` check 1 in `~/.claude/skills/ktx-l1-rewrite/SKILL.md`). Surface to operator IMMEDIATELY at session start, BEFORE dispatching this batch. Options for operator: (i) run an L1 re-extraction pass to add `k_fb_admin_only` (likely affects other `k_fb_*` cvars too -- the extraction pipeline may have a systematic gap on `RegisterCvarEx`-only registrations) THEN run this batch; (ii) draft `k_fb_admin_only` by hand in the apply pass as a special-case insert; (iii) update the Frogbot apply pass to drop the `k_fb_admin_only` See-also reference until L1 extraction catches up. Likely sibling cvars also missing: check `k_fb_*` family extraction completeness before dispatching.
