...
> **RESCAN REQUIRED before resuming this triage (added 2026-05-15).** Every verdict in the four deliverables was computed against **pre-fix L1**. Three corrections landed 2026-05-15 and all 15 ezquake tags were re-walked: cvar source-comment promotion (`2edce42b`), entity-state-retreat (`3be4d576`), F2 ghost scoping (`9a5a0c2d`). Consequence:
> - **The "no human-written prose" predicate is now wrong.** "Documented" must mean the *derived* description is non-empty (`entities.description` populated / `description_origin` set), NOT raw `help_desc` empty. Entries with `description_origin='source_inline'` are **documented from the source comment** -- do NOT re-flag them.
> - **cvar / flag_bit / token_primitive lists shrink:** comment-only entities now carry a description (cvars: the B2 "comment, no help-JSON" state is now empty -- all promoted into `help_desc`; +83 flag_bit/token_primitive now `source_inline`). Many prior `needs_doc`/`kick_to_ciscon` cvar rows are already documented.
...
> - **Action:** regenerate the queue from corrected L1 before any further triage. Expect `kick_to_ciscon` to shrink substantially for cvars. The dominant real bucket survives: ~808 source_backed ezquake cvars have neither surface -- genuinely undocumented.
