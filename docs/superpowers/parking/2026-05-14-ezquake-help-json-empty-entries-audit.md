...
> - **Action:** regenerate the queue from corrected L1 using the CORRECTED predicate above before any further triage. The dominant real bucket survives and grows: >=~808 source_backed ezquake cvars have no genuine help-JSON user-doc -- genuinely undocumented. (The arc that removes the promotion -- see the home doc's recommended action -- is a prerequisite for a clean rescan; rescanning before it lands means filtering promoted `help_json` rows by the upstream-`desc`-empty test.)
