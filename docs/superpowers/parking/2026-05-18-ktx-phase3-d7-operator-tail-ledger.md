...
[D]=dispositioned. 40 done / 3 PENDING.
NEXT = ledger row 41 = HTML#41 `_k_last_cycle_map` (synthesized).
  (HTML#40 _k_coachteam1 now [D] -- next PENDING in HTML
  order is #41.)
...
- **k_pow_pickup** (row 34): single-site, primary behaviour synthesized
  ...
  - **r39 k_overtime -- C-shaped WRONG-claim #2 (recorded session #5;
    ...
    near-misses (r36/r37) + 1 weaker near-miss (r38) + **2 traced
    counter-examples (r35 k_spw, r40 _k_coachteam1 -- both explicitly
    traced the enforcement to its body, not the label)**. C-shaped
    reasoning in 5 of the 7 session-#4/#5 synth rows; the 2 that produced wrong claims are BOTH cases where
...
- Session #5 totals so far: 2 rows -- 1 FIX (r39, meaning-inversion
  sub-class A) + 1 CLEAR (r40). Cumulative: **40 / 43 done, 3
