...
[D]=dispositioned. 39 done / 4 PENDING.
NEXT = ledger row 40 = HTML#40 `_k_coachteam1` (synthesized).
  (HTML#38 k_overtime now [D]; HTML#39 k_demoname_date [D]
  CLEAR already done -- next PENDING in HTML order is #40.)
...
- **k_pow_pickup** (row 34): single-site, primary behaviour synthesized
  ...
  - **r39 k_overtime -- C-shaped WRONG-claim #2 (recorded session #5;
    ...
    counter-example (r35)**. C-shaped reasoning now in 5 of the last 6
    synth rows; the 2 that produced wrong claims are BOTH cases where
