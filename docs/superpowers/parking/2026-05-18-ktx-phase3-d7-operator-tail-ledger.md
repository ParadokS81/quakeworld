...
[D]=dispositioned. 32 done / 11 PENDING.
NEXT = ledger row 33 = HTML#29 `timeup` (synthesized).
...
- Session #3 totals so far: 12 rows, 4 CLEAR + 6 CLEAR-fact + 2 FIX, 6
  judgment-queue adds. Cumulative: **32 / 43 done, 11 PENDING**. FIX
...
- Next: live `NEXT =` pointer in the "## Docket = 43 rows" footer
  (single source of resume truth). Currently row 33 = HTML#29
  `timeup` (synthesized).
