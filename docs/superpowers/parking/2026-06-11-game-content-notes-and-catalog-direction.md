...
> **CORRECTION 2026-06-11 (catalog-arc brainstorm pre-flight):** the "data gap" framing below DECAYED -- weapon/item/projectile stats + 41 mechanics rows WERE extracted 2026-04-27 (`id1-gameplay.yaml` -> `gameplay_*` tables; see qw-oracle `docs/arc-history.md`, "Game mechanics Layer 1 arc 1"). The L3 note family below is therefore **unblocked immediately** -- notes cite live rows (RL damage/splash/radius/refire all present, per-value cited). What is genuinely missing -- monster stats, the KTX override layer, join keys, + a completeness audit -- is scoped in `docs/superpowers/specs/2026-06-11-game-content-catalog-completion.md`. The "author rl/gl first as schema probe" sequencing FLIPS: the note consumes the catalog now; gaps it hits (first finds: splash falloff gradient, self-splash half damage) feed targeted YAML backfills.
