...
| D.2.2 | I | all 4 | `_read_extent` byte-identical across 13 live handlers (ezquake×6, fte×2, qwcl×2, mvdsv×3+2 with trivial defensive guard variants). Lowest-risk highest-volume win. | drain-in-arc (Phase 3) |
...
- ✓ `extractor_lib/` contains exactly 5 files: `__init__.py`, `_base.py`, `_resolve.py`, `_visitor.py`, `clang_config.py`. NO `handler_*.py`.
- ✓ Per-project handler counts: ezquake=8, fte=8, qwcl=3, mvdsv=7 (matches spec exactly).
- ⚠ Class-name convention: 2 fte classes lack project tag (D.5.1, D.5.2 below — same as A.1 footnote).
- ✓ All handlers extend `Visitor` except `KeynamesEzquakeHandler` (deliberate, justified in docstring).
