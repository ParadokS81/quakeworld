...
**UPDATE 2026-05-27**: The sibling arc shipped. KTX info_keys handler amended to all-sites emission with operation tagging (drops the spec 1.6 "producer-only" rule; aligns with MVDSV's existing convention). Extraction surfaces 51 info_key entities (up from 7). The 6 named user-facing keys (`kf`, `k_nick`, `k`, `k_sdir`, `postmsg`, `premsg`) shipped with rich v2-shape descriptions; 7 producer-side star keys already had synthesized descriptions; the remaining ~38 entities surfaced with stub auto-descriptions and are tracked as a sweep-triage follow-up in HANDOVER. Design decision: `docs/superpowers/parking/2026-05-27-ktx-userinfo-consumer-handler-design-decision.md`. Notable source-truth find: 7 of the 11 bits documented in `include/g_consts.h:244-256` for `kf` are stale (no live consumers); `kf` description documents only the 4 live bits + flags the dead-bit block. Memory: [[feedback_source_comments_are_hypotheses]].

**Recommendation**: track userinfo-key recasts as a sibling arc, NOT as gap closure for this arc. The KTX L1 chunked-mode dispatch arc is structurally complete at 633/633 cvar+command. **(Sibling arc partially shipped 2026-05-27; wider-sweep triage of remaining ~38 info_key entities tracked in HANDOVER Small followups.)**
...
8. **Userinfo-key pile recasts (NEW sibling arc; NOT this arc's gap closure).** **PARTIALLY SHIPPED 2026-05-27.** Mid-scope. The 6 named user-facing keys (kf, k_nick, k, k_sdir, postmsg, premsg) shipped with v2-shape descriptions; the all-sites emission rewrite surfaced ~38 additional KTX info_key entities (cross-engine reads + KTX-semantic candidates with 2-letter names) that still need sweep triage. Triage queued as a HANDOVER Small followup. Sized for follow-up: ~2-4 hours. Source: Final batch F5 misframing correction; design decision at `docs/superpowers/parking/2026-05-27-ktx-userinfo-consumer-handler-design-decision.md`.
