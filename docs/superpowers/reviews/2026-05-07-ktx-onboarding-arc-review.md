...
6. **YELLOW: orientation blob not updated for `match_event` + 9 new gameplay kinds.** Surfaced during arc-reviewer cross-check against `apps/qw-oracle/API_CONTRACTS.md` (untracked at session start; created during the arc but not by it). API_CONTRACTS' new-dataset checklist step 5 requires *"Discovery update. Edit the orientation blob and any affected tool description. If a consumer LLM cannot route to the new dataset from the catalog + orientation alone, the contract is broken."* The arc added `entities.type += 'match_event'` plus 9 new `gameplay_*.kind` values queryable via existing tools' filter parameters; `serve/mcp/src/orientation.ts` was not touched. A consumer LLM connecting to the prod MCP today gets the same orientation blob as pre-arc and doesn't know `match_event` or the new gameplay kinds exist as filter values. Soft Discovery-decay: not a data-correctness issue; tools work, but consumer routing leans on guessing rather than discovery. Effort: ~15-30 min adding 1-2 sentences to the orientation blob naming `match_event` + the gameplay-kind expansion. Recommendation: drain before prod deploy (Arc N+1 prep recommendation 1) so the orientation update ships in the same dump.
