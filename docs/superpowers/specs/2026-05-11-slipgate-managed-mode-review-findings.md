...
> A design discussion on 2026-05-12 between the operator and Claude settled most Critical findings via conversation rather than separate Pass 7 brainstorm sessions. Each Critical finding below carries a STATUS line; full settled outcomes for closed findings are captured in the *Resolution log (2026-05-12 design discussion)* section near the end of this doc.
...
- Remaining Important findings — not yet discussed; carry forward.

**Net:** all 7 Criticals are effectively settled. Next concrete actions are spec amendments (C-1 through C-7) and continued discussion of remaining Important findings.
...
## Resolution log (2026-05-12 design discussion)

> Captures the full settled outcomes for findings closed in the 2026-05-12 design discussion between the operator and Claude. Pairs with the STATUS markers on individual findings above.
...
> Updated 2026-05-12 to reflect post-discussion state. The original review's recommended sequence has been substantively overtaken; what follows reflects current reality.

### Immediate (V1 prep)

1. **Spec amendment cluster for Critical findings.** All seven Criticals are now settled in design or substantively settled in side-conversation. The amendment work is mostly mechanical; design content lives in the *Resolution log* section above.
   ...
   - **C-2:** Light↔Managed mode section. Mode marker storage, Light capability surface, mode demotion gesture, OS-level read-only blobs, updater mode-split, V1 default = Light.
   ...
   - **C-5:** Explode-everything storage model section; per-asset `role` + `origin` schema; lossless-export operationalization with byte-identical pak rebuild as the target.
...
2. **Verification spike: ezQuake `Cmd_Exec_f` path resolution (I-4).** 30 minutes. Confirms whether Class 1 delta-script can live in OS temp or must be inside the profile tree.

3. **Verification spike: FTE `cfg_save` source-walk (I-1).** Half-day. Confirms whether per-role copy-mode applies cleanly to FTE.

Spikes 2-3 can happen in parallel with the amendment cluster.

### Remaining Important findings (not yet discussed; carry forward)

- **I-3 (hub moderation latency UX)** → Arc H
- **I-5 (`.pending-swap.json` write integrity)** → Arc E
- **I-6 (profile-roles.json corruption recovery)** → Arc B
- **I-8 (multi-engine quake dirs policy)** → Arc D brainstorm + verification spike walking operator's actual dir
- **I-9 (primary-delete prompt copy)** → spec amendment cluster (mechanical wording fix)
- **I-10 (stale lockfile recovery)** → Arc A brainstorm
- **I-12 (offline role validation strictness)** → spec amendment to soften strict validation
- **I-13 (publish UX V1 surface)** → spec amendment to clarify V1 ships without publish UX
...
### Worth-a-closer-look findings

W-1 through W-10 carry into Arc A and Arc E brainstorms; each is bounded and tagged with which arc owns it.
...
The design is **ready for arc-planner after the C-1 through C-7 spec amendment cluster lands**, with the I-1 and I-4 spikes folded in. Items in "Remaining Important findings" can either be addressed before arc-planner (if quick) or carried into the relevant arc brainstorms as known-but-bounded design questions.
