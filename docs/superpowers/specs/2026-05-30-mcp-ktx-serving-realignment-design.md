...
| G2 | `search_mechanics` `kind` enum lists only the 8 original kinds; the 7 KTX kinds (`game_mode`, `mode_default`, `election_type`, `score_system`, `drop_item`, `loc_macro`, `teamplay_message`) aren't selectable. (KTX actually has an 8th kind, `death_rule` — already in the enum, but its 27 KTX rows are masked by the G1 source-default rather than the enum.) No `mode` filter. Rows omit `ruleset_gate_json`/`props_json`, so results can't even be post-filtered by mode. | `search-mechanics.ts:13`, `:49` |
