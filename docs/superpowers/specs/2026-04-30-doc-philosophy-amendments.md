...
- Anything inside `_*/` directories (directory-level seed/draft/legacy/transient marker — same rationale as the file pattern, applied at directory scope; e.g., `apps/qw-oracle/scripts/extractors/ezquake/_legacy/`).
- Anything under `.pytest_cache/` (universal Python tooling cache, parallel to `node_modules/`).

These exclusions also flow through to amendment #9's structural checks.

### Exclusion-list extension (added 2026-04-30, same arc)

The `_*` directory prefix and `.pytest_cache/` were added to the canonical exclusion list during the docs-check skill update on 2026-04-30 after the qw-oracle dry-run surfaced both as honest gaps:
- **`_*` directory prefix.** The original `_*.md` file pattern was honest at file-level but artificially narrow at directory-level. A `_legacy/` directory carries the same "draft / transient / not load-bearing" semantics as a `_legacy.md` file. Promoting to directory scope closes the gap.
- **`.pytest_cache/`.** Universal Python tooling cache, regenerated by pytest. Belongs alongside `node_modules/`, `dist/`, `build/` in the vendored / build-output exclusions. Not qw-oracle-specific.
