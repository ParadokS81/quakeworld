...
- `docs/superpowers/specs/2026-04-23-layer3-pivot-design.md` § Workstream A (enumerated the 7 items; this spec adds item 8 and turns all 8 into executable design).
...
Already decided in layer3-pivot-design § "Split `upstream_candidate` flag." This spec records the record-shape change.
...
| `cluster:ruleset-anti-script-restriction-pattern` | `commit:2dbb3f1d8d3c68e3fe984bd55684919d3263324e` (groups the 15 semantic-crossings) + shared-author/topic merge to smackdrive addition (commit 22b5b6c2) | 16: smackdrive addition + (qcon, smackdown, thunderdome) × (restrict_exec, restrict_ipc, restrict_setcalc, restrict_seteval, restrict_setex) | medium (needs cross-commit merge; semantic fallback if author+window don't match) |
...
- **Predecessor design:** `docs/superpowers/specs/2026-04-23-layer3-pivot-design.md` § Workstream A.
...
- **Handover entry being drained:** `HANDOVER.md` § Workstream A.
