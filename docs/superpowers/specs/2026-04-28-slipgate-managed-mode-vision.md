...
>
> **Pass 3 status (2026-04-29): COMPLETE.** Five sub-questions ratified and drained into the architecture spec. Six product principles landed alongside the architectural decisions:
>
> - **Manifest publish rule.** Manifest entries are recognized-role profile-content only. Unclassified files never reach a manifest. User-content (demos / screenshots / logs), library content, and private files are structurally outside profile manifests. The rule forces clean discipline -- either upgrade an asset to a known role (which is also the hub-submission gesture) or keep it out. Layer 1 closure work shrinks the unrecognized "other" bucket toward zero across releases.
> - **Bucket 7 user-library promoted from Pass 2 carry-forward to V1.** Maps, locs, and full mod gamedirs (CTF, TF, Painkeep, hipnotic, rogue) are profile-orthogonal kept content. Library has its own manifest; entries materialize into each profile's tree per `declared_gamedirs`. Profile entries take precedence over library entries at the same target_path -- per-profile loc variation handled cleanly without library schema gymnastics.
> - **Primary profile concept.** Third role pointer alongside active and launched. Default fork target, default launch fallback, mental-model anchor in UI, migration on-ramp's natural landing zone. Surfaced via "Make this primary" action with confirmation. Does NOT change GC or asset retention -- both remain refcount-driven.
> - **Capture/swap pipeline (no live read+write; observe-during-session, process-at-safe-moment).** Watcher writes to `.pending-swap.json` only during engine sessions -- no read of bytes, no hashing, no warehouse mutation. Stage 2 processing fires at engine-exit (auto), user-invoked Cleanup, or idle-nudge. `link()`-based inode-share keeps Stage 2 zero-cost on disk. Defenses against partial-file capture: never-during-session, stable-mtime, declarative integrity-check table, mod-fingerprint partial detection. Cleanup notification UX is the default flow; auto-mode opt-in routes high-confidence + integrity-pass entries automatically. Ratifies a load-bearing user property: **slipgate never opens files the engine is mid-write on.**
> - **Slipgate self-knowledge surface single-class reframed.** Earlier two-class proposal (code-bundled vs catalog-refreshable) collapsed -- most "code-bundled" tables are pure data with stable consumption contracts. Schema is the only oracle <-> slipgate coupling event; growth is continuous via per-table delta-sync. Bundled baselines keep slipgate fully functional offline; sign-in pulls deltas; `<data-root>/overrides/<table>.json` lets power users override.
> - **Hub as gravitational center.** Three-way data flow: app -> hub (submission candidates), hub -> app (delta-sync refresh), app -> app (manifests with SHA references, some hub-known, some pending). **No P2P.** Bytes flow only when the hub validates and serves them; recipients can never download a hub-unknown SHA directly. Manifests can carry placeholders for hub-unknown SHAs ("greyed-out until validated" UX); retroactive metadata enrichment activates rich UI when the hub catches up via delta-sync. Library has a separate catalog-distribution path (NOT bundled-into-profile-manifests).
>
> The body of this document remains accurate; these principles extend "What it makes possible" and "What it eliminates as concerns" without contradicting them. Pass 3 brainstorm minutes captured at `docs/superpowers/specs/2026-04-29-slipgate-managed-mode-pass3-ratifications.md`.
>
> **Pass 4 status (2026-05-05): COMPLETE.** Two sub-questions ratified and drained into the architecture spec (watcher contract refinements only; capture/swap pipeline already landed in Pass 3.4). One product principle reinforced:
>
> - **Layered defenses against partial-file capture as a slipgate promise.** Defense 1 (never process during engine session) + Defense 2 (stable-mtime check) + Defense 3 (per-extension integrity check via tiered registry) + Defense 4 (mod-fingerprint partial detection) + escalation-after-N-retries to cleanup notification together pin the user property: **slipgate never captures a half-written file as if it were complete.** Pass 4 added the per-extension declarative table (10 strict + 5 permissive + default-row fallback) and the escalation rule that surfaces persistently-unstable files for explicit user decision.
>
> Pass 4 brainstorm minutes captured at `docs/superpowers/specs/2026-05-05-slipgate-managed-mode-pass4-ratifications.md`.
>
> **Pass 5 status (2026-05-08): COMPLETE.** Three sub-questions ratified and drained into the architecture spec (5.1 + 5.2 + 5.3). Pass 5 generated substrate amendments back into Pass 1 (anchor items 1 + 3) + Pass 3.2. Seven product principles landed alongside the architectural decisions:
>
> - **Per-role materialization mode (5.1).** Configs are living files that the engine truncate-writes; configs use copy mode to preserve the content-addressed-store immutable-blob invariant. All other roles continue using hardlink. Verified via ezQuake source-walk (`src/config_manager.c` `cfg_save` / `cfg_save_onquit` / `hud_export` / `dev_dump_defaults` all use `fopen(path, "w")` truncate-write; default `cfg_backup` is 0). Cost trivial (KB-scale per profile per config).
> - **Hard-fork-with-drift-detection (5.1).** Forks remain self-contained snapshots; drift from parent is tracked via `last_synced_parent_manifest_sha`; the user explicitly accepts drift entries via the clone modal at app-open / profile-switch / engine-launch trigger points. Trade-off chosen: zero-friction overlay rejected in favor of zero-surprise drift detection. Per-role drift granularity -- cvar-level for configs (semantic three-way diff via slipgate's existing parser + oracle cvar metadata), file-level for everything else.
> - **Five-surface launch UX (5.2).** Slipgate is a quakedir manager first, engine launcher second. Five distinct surfaces (app-open / profile-switch / engine-launch / engine-exit / app-close) each carry their own UX expectations. Drift checks are non-blocking on app-open, blocking on profile-switch, light-prompt on engine-launch. Multi-instance V1 = single-instance only.
> - **Gamedir picker dropped from V1 (5.2; amends Pass 1 anchor item 3).** ezQuake handles server-driven gamedir changes natively at runtime; pre-selecting a gamedir is only relevant for offline single-player mods (mostly FTE single-player community; V1+ scope). `declared_gamedirs` stays as content-metadata for "what gamedirs this profile has content for," not "what gamedir to launch with."
> - **Verified runtime swap class taxonomy (5.1; closes Pass 1 anchor item 5).** Class 1 (mailslot console command, no engine restart -- `set` / `bind` / `alias` / `exec` / `skins` / `loadcharset` / `loadsky` / `hud_recalculate`) / Class 2 (`vid_restart` / `s_restart` / mapchange) / Class 3 (engine restart). Boundaries verified via ezQuake source-walk. Reload-cost registry as ninth slipgate self-knowledge surface table. Mailslot is ezQuake-only; FTE swaps fall back to Class 3 until FTE-IPC ships V1+.
