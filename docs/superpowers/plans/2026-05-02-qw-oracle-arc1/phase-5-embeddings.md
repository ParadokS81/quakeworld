...
- **Question:** Phase 4's migration filename. This phase reserves `006_embedding_api_log.sql`. If Phase 4 used something other than `005_layer3_concepts.sql`, ordinals collide.
  **Default chosen for now:** Assume Phase 4 uses 005. If Phase 4 chose a different ordinal, the Phase 5 drafter shifts to `00<N+1>_embedding_api_log.sql` after reading Phase 4's output. The migrator's filename-sha256 invariant handles the bookkeeping; only the literal filename string changes.
  **Who can resolve:** Phase 5 drafter, after Phase 4 lands.

- **Question:** `embedding_api_log.source` CHECK enum extends the spec. The architecture spec at `docs/superpowers/specs/2026-05-01-qw-oracle-database-architecture-design.md:320-328` names two values: `'loader'` and `'mcp-query'`. This phase adds `'verify'` as a third value to record the D8 build/query cosine-check calls (per D8: "First Voyage call on startup is a verification call (counts against `embedding_api_log`). Acceptable.") The verify call genuinely fits neither existing category - it is not a corpus-loading call (no row gets a vector) and it is not an `'mcp-query'` (it embeds a fixed probe, not a user query).
  **Default chosen for now:** Add `'verify'` per D17 (drafter best-guess when spec + decisions don't cover the case). The Phase 5 migration's leading comment names the value's purpose and Phase 6 reuses it. Per the decision-amendment rule in `decisions.md`, this should ride forward as a documented Phase 5 amendment to the spec rather than silent drift.
  **Who can resolve:** operator. Three options: (a) accept and amend the spec to name three sources; (b) reject and route verify calls under `'mcp-query'` (misleading because verify embeds a probe, not a user query, and uses both the build and query models, not just the query model - but technically harmless); (c) reject and skip logging verify calls entirely (loses the per-call latency signal but matches the spec literal). Default leans (a); if (b) or (c) is preferred, the migration text and `verify-embedding-space.ts` adjust in one edit each.
