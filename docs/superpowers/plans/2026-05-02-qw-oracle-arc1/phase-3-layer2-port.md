...
2. **Question:** Should `stats.mjs` be ported to a Postgres-flavored `stats.ts`?
   **Default chosen for now:** Not ported. The 90-line script is operator-friendly SQL wrapped in `console.log`; reproducing the exact output as `bun run stats` is one phase's worth of avoidable churn. Operator can run the equivalent psql queries directly when needed (the queries are visible in the pre-deletion file's git history if anything).
