...
| 4 | approved (orch boundary-reviewed) | `phase-4-crosslinks.md` | cvar->cvar build-time auto-link (within-codebase); source links for the 5 non-ezQuake codebases (F6 version-string handling: qtv/qwfwd omit -> plain text); entity->guide reverse-index BUILT + tested but render-SUPPRESSED in v1 via `GUIDES_PORTAL_LIVE=false` (the guides portal is a later arc -- D7 no-dead-links by construction). `phase-4-executor-prompt.md` ready | cross-links render; zero dead links |
