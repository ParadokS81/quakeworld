...
| 3 | approved (lean -- F13) | `phase-3-fanout.md` | VERIFY graceful degradation across the 5 non-ezQuake codebases (already render via 2b's generic `listSnapshots()` loaders -- F13) + thin display-name/ASCII polish. Zero new component code (D14 held) | all 6 codebases browse-able, degradation confirmed, headings match the nav |
