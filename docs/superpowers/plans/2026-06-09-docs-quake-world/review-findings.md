...
| F22 daisyUI .input/.toggle unstyled -- `@layer components` ranks below `base` so preflight strips them | SUBSTANTIVE (floor-check blocker) | F14 fix (style.css layer order) | D10, D11 |
...
## F22. daisyUI `.input` + `.toggle` render UNSTYLED -- cascade-layer order puts `@layer components` BELOW `@layer base`, so Tailwind v4 preflight strips them (surfaced in F14 operator floor-check)
