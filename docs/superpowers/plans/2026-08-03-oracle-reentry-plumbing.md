...
- **F15 -- wave-D residue (task-0 results 2026-08-04).** (a) Orphan `concepts` row `dryrun-fps-display` (empty summary, no backing file, never in git history -- a stray dry-run insert): REAL, and task 0 HALTED on it exactly per design -- `load-concepts` has NO pruning path (full code read: disk-walk + per-slug upsert; the only DELETEs are per-slug rebuild-in-place; no prune script exists repo-wide). Deletion verified structurally safe (migration 005 CASCADE FKs cover its 7 chunks; zero `concept_concepts` links either direction). Proposed minimal fix: walked-slug-set prune in load-concepts gated behind an explicit `--prune` flag (default-on would mass-delete on a partial checkout). DECISION AT GATE: approve the loader change + reconcile (recommended) vs ship the inert orphan to prod as a HANDOVER-sweep item. (b) has_note 571-vs-570 CLOSED AS MEASUREMENT ARTIFACT: the 571st player is `.devil` -- `.devil.md` exists on disk; `ls *.md` skips dotfiles, `find` doesn't. Community loaders confirmed self-reconciling by construction (has_note = pure function of wiki-snapshot content, full-row overwrite) and a zero-write `--dry-run` reproduces the exact current DB state (571/350) -- no drift ever existed. Second measurement-artifact finding of this arc (after F14). (c) Stale "no MCP resolver today" citations in two `_methodology/game-modes` docs + the stale tree-sitter line at `docs/layer1-extraction-roadmap.md:101` -> HANDOVER-sweep feed (F2 family). (d) `docs/arc-history.md:252` "12 tools" is correct AS HISTORY -- the sweep must NOT "fix" it. (e) Arc commit-log attribution is now inconsistent (wave-D commits trailer-less, see F16) -- accepted, no history rewrite.
- **F18 -- DEPLOYMENT.md pre-fence history sections carry a stale dump-glob (deferred).** Task 5 truthed the two routine sections but left "Rollback"/"Operator commands"/"Troubleshooting" as flagged pre-fence history; their Tier-2 dump-glob (`qw_oracle-*.sql`) misses the new `.dump` custom-format naming. Low pressure -- the routine corpus-refresh section now cross-references the step-4 dump as Tier-2 insurance; full rewrite of the history sections rides whatever next touches them. HANDOVER-sweep feed.
- **F17 -- qw-oracle Dockerfile workspace-shape drift (fixed 2026-08-04, orchestrator-inline).** The Dockerfile's hardcoded per-app `COPY package.json` list (its own comment: "Copy every apps/*/package.json so the shape matches the lockfile") predates `apps/docs-web` (scaffolded `945a3292`, 2026-06-11), so `bun install --frozen-lockfile` saw 4 workspace apps vs the lockfile's 5 and refused -- ANY oracle image build since docs-web landed would have failed identically; this arc merely surfaced it (task 3 first build attempt). Fix: one COPY line, alphabetical position; packages side verified complete (qw-version-resolution only). Family note: third hardcoded-list/path decay this arc (F6 .env, F16 hook path, F17 COPY list).
