...
- **Stage:** EXECUTING -- Phase 3 wave C (SDK upgrade) dispatched 2026-08-04; waves A+B + F12 drain LANDED (suite 22/22 orchestrator-verified; wave-B code commits now on origin via the docs push -- GitHub only, nothing deployed)
- **Last action:** F12 drain verified (commit `46632983`, real match_event snapshots live); wave C dispatched with D4 guardrails + server-version bump to 0.7.0
- **Next action:** wave C lands -> wave D (contract truth-up: recon's staleness list) -> orchestrator boundary gate (incl. A2 operator review)
