...
- **Stage:** EXECUTING -- Phase 3 wave B (tools implementation) dispatched 2026-08-04; wave A recon LANDED (F4 closed, F11 opened, A2 adjudicated)
- **Last action:** recon delivered the promise matrix -- parking-doc gaps all SHIPPED except describe_mode overrides (37-row surface confirmed); NEW gap F11 (four-projects description text + f965dd20 info_key doc regression); enum widening adjudicated A2
- **Next action:** wave B lands 5 scoped items -> wave C (SDK bump) -> wave D (contract truth-up: recon's staleness list) -> orchestrator boundary gate
