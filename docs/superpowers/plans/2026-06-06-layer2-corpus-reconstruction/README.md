...
**Update (2026-06-06, post Phase A ship + Phase B measurement):** Phase A shipped + independently verified green by the orchestrator (migration 021, 1008 threads all embedded, hybrid RRF rewire mirroring `search_entities`, tsc + 18/18 MCP tests clean, orientation/index/API_CONTRACTS truthed). **Operator go/no-go gate GREEN (2026-06-06):** the side-by-side (`scripts/calibration/scratch/gate-A-compare.md` -- 24/24 in-slice reverse-gen queries hit on threads vs 9/24 on session-FTS, threads dead-on where FTS returns no hits) cleared the bar and the operator formalized the go, so D2's precondition for Phase C is met (C RUN now gated only on quota reset). Phase B re-scoped (executor finding, orchestrator-ratified): the read-only gap-sweep corrected D9's backfill estimate ~25x (3h/cap750 = 18,365 agents, not ~650-750) and inverted the swept dial from chunk-cap to **lull-gap**; cap ratified at **1500** (cleaner partition than 2500), **operating point 12h/1500 = ~3,796 agents** (35 batches). **Phase C PREP shipped + verified (2026-06-06):** pipeline built (shared loader core + backfill-batch + v2 fence + ledger) and validated end-to-end on the #antilag-2026 slice (idempotency / supersede / straddle / kill-switch = KEEP-provisional); Phase A intact (1008 v1 golden). **RUN gated on quota reset (~2026-06-08)** via `phase-C-executor-prompt.md`. Full detail: `decisions.md` D9 amendment (cap-sweep result) + the version-agnostic-supersede amendment under D5 + `apps/qw-oracle/scripts/load-chat/backfill-ledger.md`.
