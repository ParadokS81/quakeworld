...
> - **Pass 6 (cloud catalog data shape, Arc H pre-implementation brainstorm): COMPLETE 2026-05-11.** Four sub-questions resolved with significant collapse. (6.1) Standalone-shareable-config dual lifecycle: catalog stores SHAs ("you got a copy, it's yours" -- flavor 1 of 3); subscribe and fork flavors deferred to V1+. All configs (not just `config.cfg`) get the Pass 2.5 500-version history ring. Two-tier delete UX. Provenance lives in version history, not on the manifest entry (no `added_via` field; recognition at display time via hub lookup). **Bundles ARE manifests** -- single primitive at all sizes (single-asset publish = one-entry manifest, bundle = multi-entry, profile = full); tags + versioning fall out of name + publisher + publish-order. Follow is post-hub-V1 (Hub V2 territory). (6.2) Bundle lifecycle in slipgate-app (NEW sub-question): bundles are first-class objects with their own UI surface; local download log carries per-file bundle lineage; apply-to-profile distributes files into natural domains via the clone modal as V1 selector primitive (now seven consumers). (6.3) Catalog metadata divergence collapsed: Pass 3.1's "configs thin / assets rich" framing replaced by single-schema-with-optional-fields. Mandatory at publish: name + publisher + per-file (sha + role + target_path); all other fields gracefully optional. Rich asset metadata (BSP details, image dims, perceptual hash) is derived/layered, not part of catalog schema. Engine compat is user-tagged, not auto-derived. Pre-seeded popular-asset details = new self-knowledge surface table candidate (10th table; V1+). (6.4) Library separate-catalog-distribution: collapsed via 6.1e. Library uses the same bundle-as-manifest primitive with `library:*` roles. Pass 6 brainstorm minutes captured at `docs/superpowers/specs/2026-05-11-slipgate-managed-mode-pass6-ratifications.md`.
>
> **ALL brainstorm passes (1-6) now CLOSED.** Next move: arc-planner for Arc A (asset warehouse substrate). Review-prep doc at `docs/superpowers/specs/2026-05-11-slipgate-managed-mode-review-prep.md` (load-bearing decisions catalog + deferred carry-forwards list); fresh-eyes review handoff at `docs/superpowers/parking/2026-05-11-slipgate-managed-mode-review-handoff.md`.
...
**Eight implementation arcs (A through H) plus a Backup sub-arc** ship Managed Mode end-to-end. Six items are V1 (the minimum coherent product surface, including the Backup sub-arc per Pass 5.3). Three items are post-V1 polish/extension.
...
V1:
  ...
  Backup. V1 backup architecture (GitHub OAuth + local-external + symmetric selector modal + snapshot-with-manifest-history; Pass 5.3 ratified)
        - Could land as a Arc B sub-section or its own sub-arc; sequencing decided at arc-planner time.

V1+ post-launch:
  F. Lossless export
  G. Version history (per-config IDE-shaped restore)
  C-full. Full profile UI (browse-as-abstraction, side-by-side diff, fork)
  H. Cloud catalog hookup (V1+ for slipgate; first hub release ships browse + download + publish only; follow is V2)
```
...
**Pass 6 outcomes (catalog data shape, ratified 2026-05-11):**
- **Bundles ARE manifests (6.1e).** Single primitive at all sizes -- single-asset publish = one-entry manifest, bundle = multi-entry, profile = full. Tags + versioning fall out of name + publisher + publish-order. No separate tag store, no separate bundle object type.
- **Single schema across single-assets and bundles (6.3b; collapses Pass 3.1's "configs thin / assets rich" framing).** Mandatory at publish: name + publisher + per-file (sha + role + target_path); all other fields gracefully optional (author, description, license, version, changelog, engine_compat).
- **Lifecycle: "you got a copy, it's yours" (6.1a).** Catalog entry and downloaded copy diverge after download. Subscribe + fork flavors deferred to V1+.
- **No `added_via` field (6.1d).** Catalog stores SHAs only; lineage shows in version history view via hub lookup. Bundle-download lineage lives in per-user local download log (slipgate-app-side, separate from manifest).
- **Engine compatibility is user-tagged, not auto-derived (6.3d).** Config converter cvar carry-over detection is the future enrichment layer for configs (V1+).
- **Library distribution collapses into bundles=manifests (6.4).** Same primitive as everything else, with `library:*` roles. No separate library-distribution mechanism needed.
- **Bundles are first-class in slipgate-app (6.2).** Own UI surface ("Bundles" left-side entry) + local download log + apply-to-profile via clone modal as V1 selector primitive.
- **Follow is post-hub-V1 (6.1f).** Hub V1 ships browse + download + publish only.

See architecture spec `Cloud catalog interaction -- Catalog data shape` and `Bundles in slipgate-app` for full detail. Pass 6 brainstorm minutes at `docs/superpowers/specs/2026-05-11-slipgate-managed-mode-pass6-ratifications.md`.
...
| Item | Status |
|---|---|
| Vision spec | [OK] Drafted + Pass 1/2/3/4/5/6 status notes drained |
| Architecture spec | [OK] Drafted + Pass 1/2/3/4/5/6 fully drained into body; reading guide added at top 2026-05-11 |
| Roadmap (this doc) | [OK] Drafted + Pass 1/2/3/4/5/6 progress block updated |
...
| Brainstorm Pass 6 (Arc H pre-implementation -- catalog data shape) | [OK] Complete 2026-05-11 |
| Spec hygiene pass + reading guide + review-prep doc + fresh-eyes review handoff | [OK] Complete 2026-05-11 |
| Fresh-eyes design review (cold reader) | Pending; runs in fresh terminal next session |
