...
**Total schema impact:** 10 CHECK widenings + 1 new table (`match_event_versions`), split across migrations 009 / 010 / 011 (renumbered from original 008/009/010 per D5 amendment; slot 008 was taken by qwiki arc). Per-value enumeration: 1 (`log_template_versions.channel` += `'logfile'`) + 1 (`entities.type` += `'match_event'`) + 1 (`gameplay_entity_defs.kind` += `'monster'`) + 7 (`gameplay_mechanics.kind` += `'game_mode'`, `'election_type'`, `'score_system'`, `'drop_item'`, `'loc_macro'`, `'teamplay_message'`, `'mode_default'`) = 10. Earlier scaffolding said "9" -- that was a meta-count error caught during Phase 7 drafting.
...
- **Stage:** EXECUTION -- Phases 0/1/2/3/4/5/5.5/6/7 shipped; Phase 8 next (end-of-arc docs sweep).
- **Last action:** 2026-05-06 -- Phase 7 (validation surface: F1 quality-grid probes + JSONB regression gate + VALIDATION-RUNBOOK.md KTX section + 5-engine cross-project audit) shipped via arc-executor as main commit `6fabeecd` (7 files, +783/-6) + housekeeping follow-on `9019d2d8` (1 file, +25/-23, runbook fixture NOT-NULL columns caught at boundary verification Probe 5; same probe-spec drift class as F29). 24 KTX F1 probes all PASS at live counts (cvar=260, command=358, info_key=7, log_template=1195, match_event=7, monster=13, game_mode=27, mode_default=317, election_type=5, death_rule=27, score_system=3, drop_item=31, loc_macro=15, teamplay_message=21). JSONB regression gate clean (17 columns, 0 string scalars). Cross-project audit produced 4 effective findings (D.1.1 ezquake doc_only_count pre-existing / D.2.1 class-name cosmetic / D.3.1 + D.6.1 gameplay_taxonomies parallel-aggregator-naive stat / D.8.1 F19 OVERVIEW verify -- already Phase 8 territory). 3 F29 anchor probe-spec drains inline (monster name `fish` -> `monster_fish`; attributes_json typeof object -> array; format_string LIKE three-tab -> XML-content match). 5 ezquake F1 FAILs surfaced are pre-existing per HANDOVER:26, NOT a Phase 7 regression. F22 doctrine fix surviving. **D16 7-for-7 streak holds functionally** with housekeeping asterisk (closest precedent: Phase 1 perf follow-on `864fdf7c`). All 7 phase-boundary probes PASS, independently re-verified by orchestrator session #4. Working-tree fence held (operator MCP-API + qwiki side-tracks untouched in both commits). New finding F29; no decision amendments.
- **Next action:** Phase 8 ready to dispatch in fresh executor terminal -- end-of-arc docs sweep. README.md / SCHEMA.md / OVERVIEW.md slim-doc Arc 1 refresh sweep (absorbs HANDOVER "qw-oracle slim-doc Arc 1 refresh sweep" backlog item per F20) + 7+ EXTRACTOR-PLAYBOOK additions including F17 dual-row design note + F25 Pattern 13 emission cross-arc invariant + F26 Pattern 6 string-literal-scope note + F27 Pattern 9 banner-coverage variability note + F28 non-Visitor / non-libclang handler patterns (full lifecycle stub list + transition-scan exclusion convention) + F29 anchor probe live-data verification discipline + the original Pre-Port Discovery Sweep + Pre-Commit Discovery Cross-Check + Handler-grouping rationale + Pattern 15 STRING_LITERAL-array walker + Pattern 10 ENUM_DECL widening + Pattern 16 X-macro file-parse note. Also verify F19 doctrine corrections survived in OVERVIEW.md / EXTRACTOR-PLAYBOOK.md / extractors/CLAUDE.md / VALIDATION-RUNBOOK.md (5 sites); 4 verified clean post-Phase-7; OVERVIEW.md is the remaining D.8.1 grep step. Mostly inline execution per `feedback_no_subagents_for_mechanical_edits.md` -- markdown sweep with full content shipped via phase MD. D16 single-commit. D19 ASCII. D20 main tree. Orchestrator session #4 drafts Phase 8 executor briefing at `/tmp/ktx-phase-8-executor-prompt.md`. Standalone Python handler `_handler_match_events.py` (438 lines) at `apps/qw-oracle/scripts/extractors/ktx/`; does NOT inherit from `Visitor`, does NOT use libclang. Four-stage flow: `_parse_xsd` (xml.etree.ElementTree over `resources/extralog/ktxlog_0.1.xsd`) -> `_grep_emissions` (multi-line regex over `items.c` + `combat.c` + `client.c` + `logs.c`) -> `_merge_emissions_into_events` -> `finalize`. 7 duck-typed Visitor lifecycle stubs (driver calls `start_file` / `end_file` / `enter_function` / `exit_function` / `enter_compound` / `exit_compound` / `visit_cursor`; phase MD listed only 5; gap drained inline). New TS loader `load-match-events.ts` (74 lines) mirrors `load-log-templates.ts`; both JSONB columns bind via `tx.json(...)` per D14. F14 anchors reproduced exactly: 7 events / 5 named complexTypes (damagetype + deathtype + backpacktype shared + poweruptype shared + mapitemtype) / 4 named simpleTypes / 13 emission sites with locked per-event distribution. **Counts in dev DB post-Phase-6:** `entities` cvar=260 + command=358 + info_key=7 + log_template=1195 + match_event=7 = 1827; `match_event_versions` = 7; `log_template` channel='logfile' XML rows = 9 (UNCHANGED -- D10/F17 dual-row holds; Phase 2 printf-handler git-clean). **D16 6-for-6 streak.** Two phase-MD infrastructure gaps drained inline (lifecycle stub list 5->7; `load-version.ts` transition-scan exclusion list `{asset_category} -> {asset_category, match_event}`); both captured as **F28** (non-Visitor handler infrastructure gaps; Phase 7 + Phase 8 ownership). **F14 third 2026-05-06 amendment** corrects the anchor body's "7 complexTypes" prose mislabel (those are 7 EVENT names; live XSD has 5 named complexTypes). All 9 phase-boundary probes PASS, independently re-verified by orchestrator session #4. Working-tree fence honored: operator's MCP-API + qwiki side-tracks remain untouched.
- **Next action:** Phase 7 ready to dispatch in fresh executor terminal -- F1 quality-grid probes for all KTX kinds (cvars / commands / info_keys / log_templates / game_mode / mode_default / election_type / death_rule / monster / score_system / drop_item / loc_macro / teamplay_message / match_event = 14 kinds) + `F1.jsonb_columns_not_strings` extension covering 5 new gameplay kinds + match_event_versions (`attributes_json` + `emission_call_sites_json`) + VALIDATION-RUNBOOK.md KTX section + cross-project audit (KTX onboarding doesn't break any prior-engine probe + F28 transition-scan exclusion-list-vs-per-type-versions-table reconciliation as new entity types land). D18 subagent matrix: F1 probes Sonnet medium / cross-project audit Opus medium. D16 single-commit. D19 ASCII. D20 main tree. Orchestrator session #4 drafts the Phase 7 executor briefing at `/tmp/ktx-phase-7-executor-prompt.md`.
...
| 1 | shipped | `phase-1-foundation.md` | `phase-1-drafter-prompt.md` | Pattern 6 cross-header lift in `extractor_lib._source` (depth-1 #include walk; D4 + F16 amended) + migrations 009/010/011 (renumbered per D5 amendment) + new `gameplay_sources` row for `'ktx'` | Schema admits all KTX content; cross-header macros resolve for any engine |
| 2 | shipped | `phase-2-pass1-entity-handlers.md` | `phase-2-drafter-prompt.md` | Pass 1 first-class entity handlers (cvars + commands + info_keys + log_templates) + 4 loader wirings + KTX dispatch wiring in `extract-tag.ts` | KTX cvars + commands + info_keys + log_templates queryable in dev DB |
| 3 | shipped | `phase-3-modes-handler.md` | `phase-3-drafter-prompt.md` | `_handler_modes.py` + `load-modes.ts` (game_mode catalog 27 rows + mode_default overlays 317 rows) | game_mode + mode_default rows queryable; mode-aware queries possible |
| 4 | shipped | `phase-4-taxonomies-handler.md` | `phase-4-drafter-prompt.md` | `_handler_gameplay_taxonomies.py` + `load-gameplay-taxonomies.ts` (election_type 5 + death_rule 27) | election_type + death_rule rows queryable; qw-event-log validation harness anchor available |
| 5 | shipped | `phase-5-tables-handler.md` | `phase-5-drafter-prompt.md` | `_handler_gameplay_tables.py` + `load-gameplay-tables.ts` (monster 13 + score_system 3 + drop_item 31 + loc_macro 15 + teamplay_message 21) | All 5 Group-B struct-array kinds queryable |
| 5.5 | shipped | (no MD; F25 disposition closure) | n/a | `_handler_modes.py` Pattern 13 emission retrofit + remove F25 serial guard in `extract.py` + parallel-vs-serial equivalence test | Modes handler runs parallel-safe; F25 closed |
| 6 | shipped | `phase-6-match-event-handler.md` | `phase-6-drafter-prompt.md` | `_handler_match_events.py` (XSD-driven; not libclang) + `load-match-events.ts` (7 entity rows + 13 emission sites) | match_event entity type populated; qw-event-log validation harness fully unblocked at schema level |
| 7 | shipped | `phase-7-validation.md` | `phase-7-drafter-prompt.md` | F1 quality-grid probes for all KTX kinds + JSONB-binding regression gate + validation runbook + cross-project audit | KTX onboarding has same auditability as the 4 prior engines |
