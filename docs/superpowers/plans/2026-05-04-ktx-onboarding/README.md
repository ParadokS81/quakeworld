...
- **Stage:** ARC SHIPPED. All 9 phases (0/1/2/3/4/5/5.5/6/7/8) complete. arc-reviewer pass next, in fresh terminal per skill structural requirement.
- **Last action:** 2026-05-07 -- Phase 8 (end-of-arc docs sweep + 7 EXTRACTOR-PLAYBOOK additions + F19/F22 doctrine survival) shipped via arc-executor as single commit `83288501` (5 files, +414/-51). qw-oracle slim-doc Arc 1 refresh sweep landed across README.md / SCHEMA.md / OVERVIEW.md (Postgres-correct + 12-tool MCP + Bun-only + Discord-only Layer 2 + Layer 3 first-class + KTX schema deltas + F20 HANDOVER backlog absorbed). EXTRACTOR-PLAYBOOK.md extended (+298 lines) with the 4 originals + Pattern 10 ENUM_DECL widening + Pattern 16 X-macro file-parse + F17 dual-row design + F25 Pattern 13 cross-arc invariant + F26 Pattern 6 string-literal-only scope + F27 Pattern 9 banner-coverage variability + F28 non-Visitor handler infrastructure + F29 anchor probe live-data discipline. F19 + F22 doctrine fix survival verified across 5 reference sites; one pre-existing tree-sitter slip in `ezquake/diagnostics/extraction-comparison-report.md` drained inline (excluded-path historical artifact predating D2). Resolves F17/F19/F20/F22/F25/F26/F27/F28/F29. **D16 8-for-8 functional streak with P1 + P7 housekeeping asterisks.** Working-tree fence held: 23 uncommitted unchanged; operator MCP-API + qwiki side-tracks untouched. No new findings; no decision amendments.
- **Next action:** arc-reviewer pass runs in **fresh terminal** per `arc-reviewer` skill structural requirement (a terminal that ran any phase has anchored expectations and cannot deliver an honest spec-vs-shipped read). Post-arc handoff at `docs/superpowers/parking/2026-05-07-ktx-onboarding-postarc-handoff.md`. Reviewer reads spec + decisions + arc-history + per-phase MDs + cross-phase memory captures cold; produces DELIVERED / DELIVERED-DIFFERENT / DEFERRED / MISSING walkthrough. Post-arc cleanup (operator-driven): delete two HANDOVER bullets ("qw-oracle slim-doc Arc 1 refresh sweep" per F20; second TBD per operator judgment); unblock qw-event-log validation harness parking doc; Layer 3 concept-note candidates parked at `2026-05-04-ktx-layer3-concept-note-candidates.md` become eligible. Standalone Python handler `_handler_match_events.py` (438 lines) at `apps/qw-oracle/scripts/extractors/ktx/`; does NOT inherit from `Visitor`, does NOT use libclang. Four-stage flow: `_parse_xsd` (xml.etree.ElementTree over `resources/extralog/ktxlog_0.1.xsd`) -> `_grep_emissions` (multi-line regex over `items.c` + `combat.c` + `client.c` + `logs.c`) -> `_merge_emissions_into_events` -> `finalize`. 7 duck-typed Visitor lifecycle stubs (driver calls `start_file` / `end_file` / `enter_function` / `exit_function` / `enter_compound` / `exit_compound` / `visit_cursor`; phase MD listed only 5; gap drained inline). New TS loader `load-match-events.ts` (74 lines) mirrors `load-log-templates.ts`; both JSONB columns bind via `tx.json(...)` per D14. F14 anchors reproduced exactly: 7 events / 5 named complexTypes (damagetype + deathtype + backpacktype shared + poweruptype shared + mapitemtype) / 4 named simpleTypes / 13 emission sites with locked per-event distribution. **Counts in dev DB post-Phase-6:** `entities` cvar=260 + command=358 + info_key=7 + log_template=1195 + match_event=7 = 1827; `match_event_versions` = 7; `log_template` channel='logfile' XML rows = 9 (UNCHANGED -- D10/F17 dual-row holds; Phase 2 printf-handler git-clean). **D16 6-for-6 streak.** Two phase-MD infrastructure gaps drained inline (lifecycle stub list 5->7; `load-version.ts` transition-scan exclusion list `{asset_category} -> {asset_category, match_event}`); both captured as **F28** (non-Visitor handler infrastructure gaps; Phase 7 + Phase 8 ownership). **F14 third 2026-05-06 amendment** corrects the anchor body's "7 complexTypes" prose mislabel (those are 7 EVENT names; live XSD has 5 named complexTypes). All 9 phase-boundary probes PASS, independently re-verified by orchestrator session #4. Working-tree fence honored: operator's MCP-API + qwiki side-tracks remain untouched.
