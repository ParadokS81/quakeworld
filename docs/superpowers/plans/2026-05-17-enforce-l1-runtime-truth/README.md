...
|---|---|---|---|---|---|---|
| 1 | drafted (awaiting review; DEVIATION OQ-1) | phase-1-track-a-callgraph-passenger.md | **Track A -- call-graph reachability passenger.** Tier-1 shared module beside `extractor_lib/_visitor.py`/`clang_config.py`; D3-D7; read-only observer, single seam, fail-safe-off (D6); commented-register surfaced as the separate feeder (D7.1) | `reachable(entity)->{yes/no,which variants}` queryable; the 3-gate known-answer probes GREEN on the mechanism's own output; existing entity JSON byte-identical (X3 zero-diff) | Automated (mechanism self-validation + zero-diff) | ~250-450k (call-graph BFS/union/address-taken closure; subagent-heavy; design task Opus MAX) |
