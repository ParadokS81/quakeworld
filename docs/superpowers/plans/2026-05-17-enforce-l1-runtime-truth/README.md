...
**Status: ARC SHIPPED 2026-05-20.** All 5 phases shipped: Phase 1
`51604f67` / Phase 2 `3c136826` / Phase 3 `895817bb` / Phase 4
`f68b045b` + F15 fix-cycle `59d34786` / Phase 5 `41965fe2` + 2026-05-20
F20 .md regen. Both North-Star directions met for ezQuake: Track A
(libclang call-graph reachability + ghost elimination -- 2 genuine-dead
cvars + 4 genuine-dead cmdline_params shipped to the upstream
delete-list; 164 build-conditional entities correctly kept with
level-2 annotation) and Track B (HUD_Register hidden-command recovery
-- 129 commands first-class in L1). Post-execution review-findings
F9-F20: all OPEN findings either RESOLVED, accepted as correct
arc behavior, or routed as deferred work; no `decisions.md`
D-amendment across the whole arc except D5 (preprocessor-only
not-compiled, F9 2026-05-17) and D7 (D7.1 commented-register feeder
separation, F4/F5 2026-05-17). Arc tag:
`arc-enforce-l1-runtime-truth-shipped`. Arc-reviewer handoff:
`docs/superpowers/parking/2026-05-20-arc-reviewer-enforce-l1-handoff.md`.
FTE / QWCL / MVDSV remain gated follow-ons (D2 / D22), a separate
future arc.

---

**Historical pre-execution status:** ALL 5 DRAFTS APPROVED 2026-05-17 --
the arc plan was FULLY DRAFTED. A pre-execution **cross-phase
consistency audit** (dedicated fresh terminal; brief at
...
no redraft, no D-amendment; operator-cleared). Scaffold built 2026-05-17
(decisions / review-findings / prerequisites / phase-template /
handoff-prompt / README).
...
draft -> review -> ship before the next). Per-phase MDs were drafted in fresh
...
|---|---|---|---|---|---|---|
| 1 | shipped (2026-05-17, commit `51604f67`; OQ-1 ratified -> D7 AMENDMENT; X3 file-list -> F6; **F9 at execution: drafter not-compiled premise refuted -> operator-ratified D5 AMENDMENT Option B, zero `_callgraph.py` change, Gate-3 expected cell corrected; orchestrator-independently-reverified 3-gate GREEN + X3 byte-identical two ways**) | phase-1-track-a-callgraph-passenger.md | **Track A -- call-graph reachability passenger.** Tier-1 shared module beside `extractor_lib/_visitor.py`/`clang_config.py`; D3-D7 (D7 amended 2026-05-17); read-only observer, single seam, fail-safe-off (D6); commented-register = minimal standalone textual scanner, architecturally separate (D7.1, feeder b) | `reachable(entity)->{yes/no,which variants}` queryable; the 3-gate known-answer probes GREEN on the mechanism's own output; existing entity JSON byte-identical (X3 zero-diff) | Automated (mechanism self-validation + zero-diff) | ~250-450k (call-graph BFS/union/address-taken closure; subagent-heavy; design task Opus MAX) |
| 2 | shipped (2026-05-18, commit `3c136826`; no deviation; **R1 GREEN -- D8 literal-only premise AST-confirmed (`nonliteral_count==0`; the prime refuted-premise candidate did NOT fire -- contrast Phase-1 F9)**; C1 copy-run hazard -> review-findings **F10** + dated Phase-2 MD orchestrator-correction (`HUD_COMMANDS_OFF=1` baseline leg; F6 precedent, no D-amendment); orchestrator independently re-verified 3 ways -- structural (D1 no-blend / R1 STOP path / R7 no-cvar / single Phase-1-consistent seam; `togglehud`+`hud.c:1281-1282` comment-trap STRUCTURALLY impossible) + primary-source by-symbol (`hud.c`/`hud.h`/`hud_radar.c` zero-drift @ `3f9e724f`) + empirical (own mktemp OFF/ON: 8 F6 stems byte-identical + 9th additive + `verify-hud-probes.py` ANCHOR1/2/3+R7+R1 GREEN exit 0); P2->P3 contract verbatim-match) | phase-2-track-b-handler-hud.md | **Track B -- `ezquake/_handler_hud.py` (commands-only).** New project-private handler; D8/D9/D10/D11-amended; bare `<name>` + `+hud_`/`-hud_`; R1 AST-confirm 0 non-literal first args; R7 zero-cvar guard; D16 element-key emission | Handler emits the bare/`+`/`-` commands; 3 HUD anchors + zero-`type=cvar` probe GREEN on handler JSON; existing entity JSON byte-identical (X3) | Automated (mechanism self-validation + zero-diff) | ~200-350k |
| 3 | shipped (2026-05-18, commit `895817bb`; DONE_WITH_CONCERNS -> F11 accept-as-is / F12 dated MD-correction / F13 floor recalibrated; **NO `decisions.md` deviation, no D re-opened**. Migration `015` [ordinal EXECUTOR-DERIVED live per F8 -- orchestrator independently re-derived 014->015, exactly one, no silent-corruption dup] lands two physically-separate nullable JSONB cols [`track_a_reachability` on cvar_versions + command_versions; `track_b_hud_recovery` on command_versions only], no CHECK, D12 structural no-blend; D14 three-slot spine; D15 feeder-tagged; D16 element-linked; slot-3 uniformly level-2 [D14/D19 boundary held; X2/W4 -- no dump cross-check, that is Phase 4]; OQ-1/OQ-2 the operator-ratified defaults; the COALESCE-on-the-3-new-cols-only upsert [every existing col keeps =EXCLUDED -- X3 row-parity; the necessary realization of D12 + the MD ordering] surfaced for arc-reviewer. **Orchestrator independently re-verified 3 ways** [executor DONE_WITH_CONCERNS treated as hypothesis -- `feedback_verify_dispatched_terminal_claims`]: structural [mig body 3 bare ALTER ADD JSONB / information_schema 3 jsonb-nullable-no-CHECK / D12 SQL=0 / COALESCE upsert read line-by-line / ordinal re-derived 015] + primary-source-by-symbol [the 3-gate JSONB round-trip EXACT incl. the F9 D5-amendment `cl_bobhead` server=`reachable` conclusion `build-excluded` UNCHANGED; `gl_outline_scale_world` commented-register/r_rmain.c:730; radar trio element-linked to one `radar` hud_radar.c:1422; slot-3 DISTINCT only `high-confidence-generalized`; cross-arc forensics -- 4 ktx-mvdsv commits during the run, none touched db/migrations or quality-grid.ts] + empirical [own F1 grid re-run -> only the 2 stale command floor probes RED; own mktemp OFF/ON X3 re-run F10-explicit -> 8 F6 stems byte-identical, 3312-10skipped == 3302 == 2788 cvar + 514 cmd; post-fix F1 FULLY GREEN 132 probes 0 regression failures; tsc clean]; P3->P4 contract verbatim match. F13 [quality-grid.ts 564->693 / source_backed 495->624; primary-source-verified legitimate D21 growth, 0 dup name_fold; `reference_qw_oracle_floor_vs_clean_reload`] is F8-scoped surgical, disjoint from the active ktx-mvdsv `F1.describe_fill.*` region) | phase-3-unified-schema-loader.md | **Unified L1 fidelity schema + loader.** D12 two separate provenance fields; D14 three-slot spine; D15 Track-A feeder-tagged per-variant evidence (R2 field-shape); D16 Track-B element-link storage (R3); D13 three-level coverage representation (slot 3 representation only) | Migration applied + SCHEMA.md updated; real Phase-1/2 output round-trips with correct provenance shape; F1 quality-grid GREEN | Automated (schema-port: migration applies, SCHEMA.md diff, F1, round-trip) | ~200-400k (schema-port + loader-port; design task Opus MAX) |
| 4 | shipped (2026-05-18; **Phase-4 RE-VERIFY orchestrator-independently-re-gated GREEN s6 -- F15 RESOLVED `59d34786`, the 3 F15-family probes CLEARED on the clean idempotent DB [command_source_state 624/7/62 NOT recalibrated, doc_only 57, cross_type_orphans 0, cmd 693], F17 surfaced + tracked NON-Phase-5-blocking; ship-record `f68b045b`, code checkpoint `702421a1`**. The acceptance contract is independently verified sound -- F7 PRIMARY embedded-SHA proxy re-run by the orchestrator incl. tamper-B trips-PRIMARY-alone, stage-1 pure subprocess COMPOSITION X2/R5, level-3 stamp EXACT [Track-A {gl_outline_scale_world,sb_qtvlist_url} / Track-B 129 / cl_bobhead build-excluded stays L2 OQ-3], CARRY-FORWARD 1 held, `F1.runtime_fidelity_shape` PASS incl. the new level-3-pinned-only leg + `F1.jsonb_columns_not_strings` PASS, F8 held [no migration; quality-grid.ts diff disjoint from F13 floor + ktx-mvdsv]. **F15 is the blocker (NOT a Phase-4 defect):** Phase-4's mandated `extract-tag --force` re-load surfaced a PRE-EXISTING Phase-3-loader source_state idempotency divergence -- 12 Track-B bare-HUD cmds flip source_backed->doc_only; orchestrator primary-source-verified NOT-Task-4-caused + a genuine "L1 idempotent" always-on-rule violation [F13-inverse -- do NOT recalibrate]; routed as its own Phase-3-loader-idempotency fix-cycle. F14+F12 orchestrator-applied dated MD-corrections [F6/F10/F12 family, no D-amendment]. OQ-1/2/3/4 operator-ratified RESOLVED. Prior approved-recon record [session-4: pin both legs `3f9e724f`, R6 74/92/129, S1/S2/F7] preserved in git history + review-findings F7. **F15 fix-cycle SHIPPED `59d34786` + Phase-4 RE-VERIFY orchestrator-gated GREEN -> Phase 4 SHIPPED. Next: Phase 5 (the LAST phase).** F17 (pre-existing Phase-3-loader fail-safe-completeness gap, autonomous level-3 tier provably protected, NON-Phase-5-blocking) in review-findings F17 + HANDOVER; full RE-VERIFY retrospective in `apps/qw-oracle/docs/arc-history.md`. Resume cold (fresh orchestrator terminal): `docs/superpowers/parking/2026-05-18-enforce-l1-runtime-truth-execution-orchestrator-resume-s6-to-s7.md`) | phase-4-acceptance-contract.md | **Acceptance contract.** D17 one shared 3-stage shape per-track; D18 hard/all-or-nothing/loud one-time-per-fork validation gate = composition of Phase-1/2 probes (R5); D19 dump-as-overriding-answer-key + version-pin sanity-proxy hard sub-gate (R6); stage-3 route-by-level; D22 per-fork-per-track precondition + off-by-default toggle | Harness passes LOUD/green at HEAD `3f9e724f`; broken-pin -> zero level-3 proven; toggle-off == today's pipeline | Automated within operator-set bounds (harness pass/fail at the pinned commit) | ~250-400k |
| 5 | shipped (2026-05-19, commit `41965fe2`; 2026-05-20 BOUNDARY-FLIP + POST-SHIP correction: **F18** dated-MD-correction (legitimate D21-ship consequence -- post-Track-B-ship `command reverse` = 0, the 129 are now first-class; no banked `front1-diff.sh` weakening; F13/F12/F14 narrative-preserved precedent) + **F19** ACCEPT-arc-correct (gl_program_sky CORRECTLY build-excluded -- registered `glc_main.c:36-44` + read `glc_sky.c:206/:368`, classic-GL default-ON `CMakeLists.txt:11`, extractor defines both renderer macros `clang_config.py:58-59`; first framing "model omits renderer dim" primary-source-REFUTED + corrected in place narrative-preserved -- the verify-don't-explain-away discipline applied to my own finding; D3 conservatism vindicated) + **F20** cmdline-liveness-arc-trigger (the prior hand-authored Class 3 cmdline-consumer-presence feeder carried verbatim per D7/OQ-1 was `.c`-only-scoped; 5 of 11 shipped Track-A entries are LIVE via `.h` macro wrappers fanning into `.c` call sites -- `sv_ccmds.c:1821/1861`, `pr2_exec.c:56`, `sv_demo.c:1848`, `R_DebugProfileContext` GL fan-out; the libclang call-graph this arc built would have caught these had Class 3 run through it -- the bug is in the prior artifact, NOT this arc's mechanism). **Visible artifact CORRECTED this terminal:** `_runtime_dead_entities.py:_CLASS3_BLOCK` trimmed to 4 verified-dead rows + `.c`+`.h` methodology accuracy + bonus tidy-up dropped + `_ROUTING` prose updated; the .md regenerated mechanically to 6 entities (1 cvar callgraph + 1 cvar commented-register + 4 cmdline_params verified zero-consumer in `.c` AND `.h` under `src/`). L1 mislabeling of the 5 cmdline_params persists until the parked cmdline-liveness sibling arc mechanizes the feeder with proper `.c`+`.h` scope; F20 is the trigger. ZERO impact on D20 Output 1 always-on signal / D21 Track-B 129 first-class commands; only Output 2 (the .md) affected; **no `decisions.md` D-amendment** -- D7/OQ-1 carry-verbatim correct in principle, the prior artifact itself overclaimed; D5/D3/D20 untouched. Prior approved-recon record [2026-05-17 session-5: X8/W2 sanity gate RE-RUN 74/92/129 both legs PASS, pin both legs `3f9e724f`, OQ-1/OQ-2 operator-ratified, decisions/findings walk clean, Opus-MAX Task-1 grade precedent-consistent] preserved in git history + scaffold. Arc tag `arc-enforce-l1-runtime-truth-shipped` pushed. Arc-reviewer handoff: `docs/superpowers/parking/2026-05-20-arc-reviewer-enforce-l1-handoff.md`) | phase-5-application-outputs.md | **Application outputs.** D20 Track-A two outputs: always-on per-version L1 signal over the 74-cmd/92-cvar pool + narrow level-3-only feeder-tagged delete-list REGENERATING `ezquake-runtime-dead-entities.md` (R4; build-excluded never in it); D21 Track-B recovered commands first-class, element-linked, level-stamped, nothing withheld | Delete-list regenerates byte-shape vs the in-repo artifact; L1 signal populated over the full banked pool; recovered commands present at level-2/3; F1 GREEN | Automated (byte-shape regen + populated signal) + operator review of the PR-ready artifact | ~200-350k |
