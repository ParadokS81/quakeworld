...
- DB schema migrated by `maintenance/update.php`: 39 `smw_*` tables present in `qwiki_beta` (Page Forms is wikitext-side only -- no `pf_*` tables); core MW table count up from ~59 (Phase 1) to 97 (Phase 1 + Phase 2 SMW tables + minor MW core column adds during update.php). (Prediction in the original draft was ~16 smw_ / ~74-80 total; corrected post-deploy 2026-05-14 against live count.)
