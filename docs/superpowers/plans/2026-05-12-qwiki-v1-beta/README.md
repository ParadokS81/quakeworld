...
**Status:** Substrate cluster (Phases 1-4) drafted. **Phases 1, 2, 3 SHIPPED 2026-05-14 at https://wiki.slipgate.me**. Phase 3 surfaced F7 (composer merge-plugin path visibility), F8 (PluggableAuthLogin URL form -- query-string vs path-info), F9 (OIDC access-token storage shape -- switched hook to bot-mode); all closed. Phase 4 awaiting per-phase approval. Phases 5-8 (Modes vertical slice) deferred until substrate ships (operator decision 2026-05-13; rationale: redraft risk from substrate implementation surprises -- Phase 3 confirmed the value of caution).

---

## Where we are right now

- **Stage:** Phase 1 + Phase 2 + Phase 3 shipped 2026-05-14; Phase 4 awaiting per-phase approval; Phases 5-8 deferred.
- **Last action:** 2026-05-14 -- Phase 3 shipped (PluggableAuth v7.5.0 + OpenIDConnect v8.3.0 + Discord OAuth via wiki.Quake.World bot + `wiki-contributor` / `wiki-curator` MW groups + D5 namespace gates). V_AUTH1 / V_AUTH2 / V_AUTH3 / V_AUTH4 / V_AUTH5 / V_OPS1 PASS; V_AUTH6 (revocation) deferred to Phase 4 spot-check. Findings F7 / F8 / F9 surfaced + closed at execution (jumbojett direct-require, PluggableAuthLogin path-info URL form, bot-mode role sync). Two cosmetic carry-forwards to Phase 4: Discord-username extraction (OIDC `name`/`preferred_username` fallback yields literal "User") and `MediaWiki:Group-wiki-{contributor,curator}-member` interface-message pages.
- **Next action:** Operator approval to begin Phase 4 (quality-tag categories + Layer 3 harvest verification per D18). When approved, dispatch fresh executor terminal with the Phase 4 prompt.
...
| 2 | **shipped** | substrate | [phase-2-drafter-prompt.md](phase-2-drafter-prompt.md) | [phase-2-extensions.md](phase-2-extensions.md) | Page Forms 5.8.1 + Semantic MediaWiki 6.0.1 + F6 LOAD list (ParserFunctions + Cite + CategoryTree + TemplateData) | LIVE 2026-05-14 -- V_PF1 / V_PF2 / V_SMW1 / V_SMW2 / V_OPS1 PASS at https://wiki.slipgate.me |
| 3 | **shipped** | substrate | [phase-3-drafter-prompt.md](phase-3-drafter-prompt.md) | [phase-3-auth-groups.md](phase-3-auth-groups.md) | PluggableAuth + OpenIDConnect + Discord OAuth (bot-mode role sync via wiki.Quake.World bot) + `wiki-contributor` / `wiki-curator` + D5 namespace restrictions | LIVE 2026-05-14 -- V_AUTH1 / V_AUTH2 / V_AUTH3 / V_AUTH4 / V_AUTH5 / V_OPS1 PASS at https://wiki.slipgate.me |
