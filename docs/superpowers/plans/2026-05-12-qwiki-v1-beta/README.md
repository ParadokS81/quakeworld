...
- **Stage:** Phase 1 + Phase 2 shipped 2026-05-14; Phase 3 approved + executor prompt drafted; Phase 4 awaiting per-phase approval; Phases 5-8 deferred.
- **Last action:** 2026-05-14 -- Phase 2 shipped clean (Page Forms 5.8.1 + Semantic MediaWiki 6.0.1 + F6 LOAD list: ParserFunctions / Cite / CategoryTree / TemplateData) with V_PF1 / V_PF2 / V_SMW1 / V_SMW2 / V_OPS1 PASS. Three in-flight findings (F4 composer platform-req / F5 PF FormEdit URL semantics / F6 bundled-ext audit) RESOLVED + closed. Phase 3 executor prompt drafted at `docs/superpowers/parking/2026-05-14-qwiki-v1-beta-phase-3-executor.md` with F4 + F6 carry-forwards baked in. Memory entries landed (`reference_mw_bundled_extensions_loading_pattern.md`, `reference_mw_composer_platform_reqs.md`, `feedback_drafter_subagent_verification_checklist.md`).
- **Next action:** Operator completes Phase 3 prerequisites (Discord OAuth app + `@wiki-beta` role + user-ID + role-ID per `prerequisites.md` Discord OAuth section). When prerequisites are green, dispatch fresh executor terminal with `@docs/superpowers/parking/2026-05-14-qwiki-v1-beta-phase-3-executor.md`.
...
| 3 | approved | substrate | [phase-3-drafter-prompt.md](phase-3-drafter-prompt.md) | [phase-3-auth-groups.md](phase-3-auth-groups.md) | PluggableAuth + Discord OAuth + `wiki-contributor` / `wiki-curator` + namespace restrictions | Discord login flow works; contributor lands in right group; namespace gates enforce |
