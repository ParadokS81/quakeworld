...
- Discord-role-sync helper `qwikiBetaSyncDiscordRole()` inline in `LocalSettings.php`; hooked into `LocalUserCreated` (first login) + `UserLoggedIn` (subsequent logins); calls `https://discord.com/api/users/@me/guilds/<DISCORD_GUILD_ID>/member` and toggles `wiki-contributor` group membership based on whether the response's `roles[]` includes `DISCORD_WIKI_BETA_ROLE_ID`.
- `apps/qwiki-sandbox/deploy/composer.local.json` extended with the OpenIDConnect merge-plugin path entry.
