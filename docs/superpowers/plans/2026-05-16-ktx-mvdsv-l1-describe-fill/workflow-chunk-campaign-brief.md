...
- **214/347 done; 133 remaining.** Buckets: cvar 95, command 27, cmdline_param 11 (info_key 45/45 DONE).
- Synthesized-origin rows: 112 (DB-verified). Last chunk: 4 `server-control-logging` (25 commands). In-scope MVDSV fingerprint now `9cb713c7`.
- Download cluster 4/8 (skins/sounds/demos/pakmaps remain -- fold into chunk 8 or a quick warm-up).
- Stragglers outside the chunk-4/5 named lists (verify-and-sweep at chunk 8/cleanup): `say` (SV_ConSay_f), `floodprotmsg` (SV_Floodprotmsg_f), `svadmin` (SV_Admin_f).
- Chunk-size: chunks 1+2+3+4 ran at 10+15+14+25, all clean. Chunk 4 scaled to 25 with one HG1 re-dispatch (recovered). SCALE further if confident. Next per plan: chunk 5 commands script/cvar-meta + cheats + web (~26) -- NOTE `if` + `localcommand` are in the normal-rcon blocklist (see Commands rule block).
