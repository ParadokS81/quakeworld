...
- **289/347 done; 58 remaining.** Buckets: cvar 55 (incl. 2 `sv_antilag*` OUT/D10 -> 53 fillable), command 3 (stragglers only), cmdline_param 0 -- DONE (info_key 45/45 DONE).
- Synthesized-origin rows: 187 (DB-verified). Last chunk: **8a** `gameplay-limits` (26 cvars). In-scope MVDSV fingerprint now `b64a5ca2`.
...
- Chunk-size: chunks 1..7 ran at 10+15+14+25+24+11+14, all clean (chunks 3+5+6+7 HG1-clean on the FIRST wave; chunk 4 needed one re-dispatch). SCALE if confident. **Chunk 8 IN PROGRESS** -- 79 in-scope NULL cvars (81 minus 2 `sv_antilag*` OUT/D10) split **26/27/26 + a 3-command tail** (say/floodprotmsg/svadmin). **8a `gameplay-limits` DONE** (26 cvars, fp `b64a5ca2`, HG1-clean first wave, 5 C-NEAR-MISS surgically fixed, +8 findings #41-#48). NEXT: **8b `networking-rate-download-logging`** (27: download remainder + rate/ping/logging/net-addr/system), then **8c `spectator-voip-mod-system`** (26), then the command tail. Buckets + canary pool + 8b/8c rule-block facts staged in `chunk8-orchestration-scratch.md`. cvar evidence base (no cmdline rule block; cmdline_param bucket DONE).
