...
- **249/347 done; 98 remaining.** Buckets: cvar 95, command 3 (stragglers only), cmdline_param 0 -- DONE (info_key 45/45 DONE).
- Synthesized-origin rows: 147 (DB-verified). Last chunk: 6 `cmdline-params` (11 cmdline_params). In-scope MVDSV fingerprint now `51eb3d5d`.
...
- Chunk-size: chunks 1..6 ran at 10+15+14+25+24+11, all clean (chunks 3+5+6 HG1-clean on the FIRST wave; chunk 4 needed one re-dispatch). SCALE further if confident. Next per plan: chunk 7 version/build cvars (`qwm_*`/`qws_*`, ~14, low-risk affirm-heavy -- biggest confident chunk), then chunk 8 behavioral/networking/spectator/voip remainder (~55, proven cvar shape, lightest-touch; SPLIT if needed). Both are cvars -- use the cvar evidence base (no cmdline rule block; cmdline_param bucket is DONE).
