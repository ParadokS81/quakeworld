...
- **238/347 done; 109 remaining.** Buckets: cvar 95, command 3 (stragglers only), cmdline_param 11 (info_key 45/45 DONE).
- Synthesized-origin rows: 136 (DB-verified). Last chunk: 5 `script-meta-cheats-web` (24 commands). In-scope MVDSV fingerprint now `4a8c453f`.
...
- Chunk-size: chunks 1+2+3+4+5 ran at 10+15+14+25+24, all clean (chunks 3+5 HG1-clean on the FIRST wave; chunk 4 needed one re-dispatch). SCALE further if confident. Next per plan: chunk 6 cmdline params (`-port/-game/-ip/-basedir/+gamedir/-d/-g/-t/-u/-no*`, 11) -- NEW shape (cmdline rule block above); small, cryptic single-letters = discovery risk. Then chunks 7-8 (cvars: version/build `qwm_*`/`qws_*`, then the behavioral/networking/spectator/voip remainder).
