...
- **342/347 done; 5 remaining.** Buckets: cvar 2 (both `sv_antilag*` OUT/D10 -- cvar bucket DONE), command 3 (stragglers only), cmdline_param 0 -- DONE (info_key 45/45 DONE).
- Synthesized-origin rows: 240 (DB-verified). Last chunk: **8c** `spectator-voip-mod-system` (26 cvars). In-scope MVDSV fingerprint now `5c298ef6`.
...
- Chunk-size: chunks 1..7 ran at 10+15+14+25+24+11+14, all clean (chunks 3+5+6+7 HG1-clean on the FIRST wave; chunk 4 needed one re-dispatch). SCALE if confident. **Chunk 8 IN PROGRESS** -- 79 in-scope NULL cvars (81 minus 2 `sv_antilag*` OUT/D10) split **26/27/26 + a 3-command tail** (say/floodprotmsg/svadmin). **8a + 8b + 8c DONE** -- all 79 in-scope cvars filled (fps b64a5ca2 -> daa50516 -> 5c298ef6; HG1-clean first wave all three; +20 findings #41-#60; download cluster 8/8). **cvar bucket DONE** (only the 2 `sv_antilag*` remain, OUT/D10). NEXT + LAST: the **3-command tail** (say/floodprotmsg/svadmin, shape=command) -> then command bucket DONE. Buckets + canary pool + 8b/8c rule-block facts staged in `chunk8-orchestration-scratch.md`. cvar evidence base (no cmdline rule block; cmdline_param bucket DONE).
...
- [chunk 8c spectator-voip-mod-system, 26 cvars] PROVEN cvar shape, the catch-all remainder; HG1 clean FIRST wave (4 canaries; added a NEW spectator-themed sv_spectatormaxspeed C-FIX to the cold-fodder pool). HEAVIEST C-FIX count of the campaign: 4 C-FIX + 3 C-NEAR-MISS, ALL 7 confirmed REAL (zero FP), ALL surgical -- each a single defect with a clear V-pass fix direction, so even at 4 C-FIX no description was fundamentally-broken -> no seeded re-synth (the surgical-vs-reseed line is "is the CORE identity/mechanism intact?", not "is it a C-FIX?"). The 4 C-FIX: sv_onDemoRemove named non-existent rmdemo/rmdemonum (those are help-text Con_Printf strings; the real commands are sv_demoremove/sv_demonumremove at sv_demo.c:1947 -- the chunk-5 "trust the Cmd_AddCommand registration, NOT the usage-string" trap recurs on a cvar's trigger-command clause); sv_reliable_sound polarity INVERTED (it is opt-OUT via rsnd 0, not opt-in -- invisible without following cl_reliable_key -> SV_MulticastEx sv_send.c:496 -> Info_Get absent-key returns ""); sv_mod_msg_file "tab-style"->backslash-delimited + "frag log"->the DISTINCT mod-frag log (modfraglogfile); sv_specprint "sp overrides" wrong on TWO axes (it is a master AND-gate not an override; and centerprint/stuffcmd check the FOLLOWED player's spec_print bit, not the spectator's -- a real engine bug, finding #55). 3 C-NEAR-MISS: sv_debug_usercmd OFF-state misses the per-client sv_usercmdtrace path, sv_kickuserinfospamcount counts setinfo INVOCATIONS not value-changes, sv_sayteam_to_spec omits the `$\` location-marker co-gate (sv_user.c:1897 !fake). F-D6a 22/26 literal + 4 ref fixes (sv_mod_msg_file consumer sv_send.c:332 -> loader sv_mod_frags.c:52; camelCase C-vars sv_ondemoremove/sv_onrecordfinish read OK; 1 prefix). Read-only rule held (serverdemo + version CVAR_ROM -> engine read-only). +26 synthesized (214->240); cvar NULL 28->2 -> **CVAR BUCKET DONE** (only sv_antilag* OUT/D10). +6 findings (#55-#60: sv_specprint wrong-client bug, sv_allowlastscores connectionless-only, vip_values *VIP cross-mod, pext_ezquake_verfortrans cross-engine alpha-gate, sv_mod_msg_file id-index OOB, sv_voip_record spectator-hear #if 0). fp 5c298ef6; idempotency stable (26 skipped-terminal); quality-grid green.
