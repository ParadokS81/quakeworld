...
- **263/347 done; 84 remaining.** Buckets: cvar 81, command 3 (stragglers only), cmdline_param 0 -- DONE (info_key 45/45 DONE).
- Synthesized-origin rows: 161 (DB-verified). Last chunk: 7 `version-build-identity` (14 cvars, `qwm_*`/`qws_*`). In-scope MVDSV fingerprint now `2333be4d`.
...
- Chunk-size: chunks 1..7 ran at 10+15+14+25+24+11+14, all clean (chunks 3+5+6+7 HG1-clean on the FIRST wave; chunk 4 needed one re-dispatch). SCALE if confident. Next per plan: chunk 8 behavioral/networking/spectator/voip remainder -- live cvar `description IS NULL` is now **81** (the plan table's ~55 is stale; re-derive the exact set from the recon query). Proven cvar shape -- **SPLIT into 2-3 sub-chunks** (81 >> the 25/24/11/14 of chunks 4-7). Skip `sv_antilag*` (OUT per the brief, handled separately) wherever they fall; fold in the download cluster 4/8 remainder (skins/sounds/demos/pakmaps). Use the cvar evidence base (no cmdline rule block; cmdline_param bucket DONE).
