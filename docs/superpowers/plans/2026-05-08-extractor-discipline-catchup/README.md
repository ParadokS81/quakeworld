...
- **Stage:** Phase 2 SHIPPED; all 5 projects reproducible (ezquake / fte / qwcl / mvdsv / ktx all PASS). Phase 3/4 still need execution.
- **Last action:** 2026-05-08 -- Phase 2 executor halted DONE_WITH_CONCERNS. Universal `reproducibility-check.ts` shipped; dispatcher case wired; 5-project audit all PASS. One drain-now fix: git diff scoped to output/ dir (phase MD used repo-wide diff; would have false-failed with uncommitted tree). One drain-now fix: `extractResult.stderr` optional chaining (Bun.spawnSync type not narrowed by stderr:'pipe').
- **Next action:** Phase 3 (parallel-vs-serial pytest) and Phase 4 (migration probes) are independent; can proceed in either order. Check if drafter prompts have already produced phase MDs.
...
| 2 | shipped | `phase-2-reproducibility-probe.md` | Universal `scripts/load-knowledge/reproducibility-check.ts` (packages VALIDATION-RUNBOOK Section 1.1 as runnable); `case 'reproducibility-check':` in `index.ts`; per-project config dict (source roots, optional `--workers`); 5-project audit findings inline | `bun run load-knowledge -- reproducibility-check --project <p>` re-runs `extract.py` and asserts empty `git diff --stat HEAD` on output |
