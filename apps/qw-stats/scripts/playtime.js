...
async function getPlaytime(label, nameCondition) {
    ...
    console.log(`  ----`);
    console.log(`  TOTAL  ${String(totalGames).padStart(4)} games   ${String(totalMins).padStart(5)} min   (${totalHours} hrs)`);
}

async function main() {
    await getPlaytime('zero (• zero)', `gp.player_name_normalized = '• zero'`);
    await getPlaytime('ParadokS (• ParadokS + • ParadokS •)', `gp.player_name_normalized IN ('• paradoks', '• paradoks •')`);
    await pool.end();
}
