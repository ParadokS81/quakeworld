...
4. Builds a curated include list -- **only the minimal viable set of files that define a setup**:
   - [OK] `configs/*.cfg` -- all saved configs
   - [OK] `autoexec.cfg`, `config.cfg` at the root
   - [OK] Custom `.pak` / `.pk3` content (anything not in the vanilla distribution)
   - [OK] Custom textures, skins, HUD overlays, crosshair packs
   - [OK] Key settings files the user explicitly opts into
