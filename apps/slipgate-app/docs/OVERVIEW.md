...
- **Clients** -- active. `ClientsDomain.tsx`. Hosts client setup: list view (one card per recognized client; today only ezQuake) and detail view with **Installation** (path picker, config dropdown, player name) and **Versions** (the Phase 3 VersionWarehouse with Switch / Delete / Add Quake client). **3.5b:** the Add-Client button now opens an inline `AddClientPanel` (replacing the Versions card content with the bulk-import flow until the user closes it).
...
- `version_warehouse.rs` -- content-addressed binary store under `<data-root>/binaries/`. Blobs at `blobs/<sha256>.exe`, per-version manifests at `<client>/<version>/manifest.json`, **3.5b adds variant nesting at `<client>/<version>/variants/<variant>/manifest.json`**. Top-level `index.json` tracks active version per `active_key(client, variant)` -- bare `<client>` for vanilla, `<client>:<variant>` for variants -- so vanilla and glsl variants of the same version have independent active pointers (separate canonical filename slots). `register_version_at` takes optional `variant`; `list_warehoused_versions_at` walks both vanilla and variants/ subdirs. Warehouse keys are lowercase (`ezquake`); user-facing display strings use `ClientDef.name` casing (`ezQuake`).
...
- `version_swap.rs` -- the single swap path. `swap_active_version` runs an `is_process_running` guard, applies the foreign-exe backup heuristic (warehouse-known bytes don't get backed up; foreign bytes rename to `<stem>.bak.exe`), copies blob to `<canonical>.new`, atomic-renames into place. **3.5b adds optional `target_variant`** -- match by `(client, version, variant)`, write to `active_key(client, variant)`. `delete_warehoused_version` refuses if active, removes the per-version manifest dir or just the variant subdir as appropriate, GC's the blob if no other manifest references it. The updater stages downloads under `<data-root>/binaries/.staging/`, calls `register_version` (variant=None -- updater installs canonical), then `swap_active_version`.
- `client_fingerprint.rs` (**NEW 3.5b**) -- PE StringFileInfo + VS_FIXEDFILEINFO numeric reader; classifies binaries as ezQuake / unezQuake-family / FTE / Unknown. Variant detection per family (FTE arch via `qwsv` substring + `32`/`64` no-hyphen suffix; ezQuake glsl via `-glsl` hyphen suffix). Family -> canonical-filename mapping (`fteqw{variant}.exe` no-hyphen for FTE; `<family>-<variant>.exe` hyphenated for others). Three Tauri commands: `fingerprint_exe`, `fingerprint_folder`, `scan_clients_in_dir` (Unknown-filtered).
- `release_cache.rs` (**NEW 3.5b**) -- per-(client, channel) GitHub Releases cache at `<data-root>/release-cache/<client>-<channel>.json`; 24h TTL. `distribution_for(client, channel)` returns the right shape (GitHubReleases / BuildsQuakeworld stub / FteTripToHell stub). `matches_official_release` normalizes via `parse_pe_version` so PE FileVersion `3.6.6.7949` matches GitHub tag `3.6.6`. Stays parallel to `updater.rs`'s existing fetcher per D3.
- `bulk_import.rs` (**NEW 3.5b**) -- `bulk_import_clients` orchestrator: per-row canonicalize-rename -> register (variant-aware) -> swap_active_version for primary (D12). `claim_as_primary` flag echoed back so the frontend writes `setups[0].quake_dirs[]` per D9 (Rust-side intentionally doesn't touch the profile JSON). `rename_to_canonical` exposed standalone for future UI flows.
- `VersionWarehouse.tsx` (frontend) -- per-client version list with switch + delete buttons, foreign-exe Import affordance at top when reconcile didn't match the current path. **3.5b:** wired Add-Client button (`onAddClient` prop), variant badges on rows, `active_key`-aware row state (vanilla and glsl rows have independent active pointers), per-row `targetExeFor()` resolves `ezquake-glsl.exe` etc.
- `AddClientPanel.tsx` (**NEW 3.5b**) -- bulk-import flow inside ClientsDomain. Folder/exe picker -> `scan_clients_in_dir` -> default-select-all checklist with primary-radio + canonicalize-consent toggle per row + dir-targeting indicator (D9 dispatch: no-primary / matches-primary / foreign-refuse) -> `bulk_import_clients` on submit. Auto-resolves canonical-rename collisions: when N rows target the same canonical filename, only one row's preview shows `will_rename`; siblings show `slot_occupied` so the orchestrator's atomic-rename invariant holds.
- `ClientImportRow.tsx` (**NEW 3.5b**) -- single-row component. Three-tier identity surface (`verified` / `unrecognized` / `stub` per F14). Per-row canonicalize-rename preview text + toggle. Reused for any future bulk-classification UI.
...
| `swap_active_version` | quake-dir/swap.ts | Copy a warehoused version's blob into the user's quake dir as the canonical exe; foreign-exe backup heuristic; refuses if target client is running. **3.5b:** takes optional `target_variant` so glsl variants swap to `<family>-<variant>.exe`; updates `index.active[active_key(client, variant)]` |
| `delete_warehoused_version` | quake-dir/swap.ts | Remove a warehoused version's manifest dir; GC the blob if unreferenced; refuses if active. **3.5b:** takes optional `variant` so vanilla and variant manifests are deleted independently; vanilla manifest deletion preserves sibling variants under `<version>/variants/` |
| `fingerprint_exe` | quake-dir/clientFingerprint.ts | **NEW 3.5b.** PE-strings + numeric VS_FIXEDFILEINFO classify a single exe as ezQuake / unezQuake-family / FTE / Unknown with version + variant detection |
| `fingerprint_folder` | quake-dir/clientFingerprint.ts | **NEW 3.5b.** Walk a dir, fingerprint every `.exe` (skips `.exe.db` / `.exe.pdb`), return `[(path, ClientFingerprint)]` rows |
| `scan_clients_in_dir` | quake-dir/clientFingerprint.ts | **NEW 3.5b.** `fingerprint_folder` with server-side Unknown filter (D7) so AddClientPanel never renders qizmo / wget / unrecognized binaries |
| `get_release_cache` | quake-dir/releaseCache.ts | **NEW 3.5b.** Per-(client, channel) GitHub Releases cache at `<data-root>/release-cache/<client>-<channel>.json`; 24h TTL |
| `refresh_all_release_caches` | quake-dir/releaseCache.ts | **NEW 3.5b.** Iterate the known (client, channel) pairs and refresh each |
| `bulk_import_clients` | quake-dir/bulkImport.ts | **NEW 3.5b.** Orchestrator: per-row canonicalize-rename -> register -> swap_active_version for primary (D12). D9 dispatch on `claim_as_primary` echoed back so frontend can populate `setups[0].quake_dirs` |
| `rename_to_canonical` | quake-dir/bulkImport.ts | **NEW 3.5b.** Standalone helper for any UI that wants a one-shot rename without bulk-import semantics |
