...
- **Success Criteria:**
  - [OK] Right drawer shows a single unified team list (TeamBrowser) with search input and filter buttons
  - [OK] Division filter buttons (Div 1 / Div 2 / Div 3) work as existing toggles
  - [OK] New "Fav" filter button filters the team list to show only starred/favorited teams
  - [OK] Star toggle on each team card works via touch (tap) -- same as desktop
  - [OK] Team selection (tap card) works and triggers comparison grid updates
  - [OK] FavoritesPanel and FilterPanel are hidden in the right drawer on mobile (CSS in input.css)
  - [OK] FAB repositioned above bottom bar (CSS: .selection-action-btn { bottom: 3.5rem })
  - [OK] Toast container non-colliding (top-4 right-4)
  - [OK] Touch targets meet 44px minimum in right drawer
  - [OK] Desktop layout unchanged
  - [OK] Drawer interactions <50ms (pure DOM/state)
...
DEPENDENT SECTIONS:
- Slice 10.0a: CSS foundation + mobile drawer HTML containers [OK]
- Slice 10.0b: MobileLayout.js DOM relocation + drawer open/close [OK]
- Slice 10.0c: Bottom bar with right drawer toggle button [OK]
- Slice 10.0d: Pointer events + swipe gestures [OK]
- Slice 3.1: TeamBrowser component + TeamBrowserState [OK]
- Slice 3.2: FavoritesService (getFavorites, isFavorite, toggleFavorite) [OK]
- Slice 3.3: FilterPanel + FilterService (min player filters) [OK]
