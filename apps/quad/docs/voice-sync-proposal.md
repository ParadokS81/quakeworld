...
2. **Multiple events per match**: 4 lifecycle events (countdown, start, end, map_end) x 2 packets = 8 packets per map. Probability of losing ALL packets for even one event: astronomically low.
...
**Relay storage**: Stateful -- events stored for 30 days. This enables reconnect backfill (bot missed events during a brief disconnect), a live match dashboard, and historical analytics. Storage is trivial: ~200 servers x ~10 events/day x 1KB = ~2MB/day.
