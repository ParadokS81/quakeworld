...
**Locked:** 2026-05-28. Anchored to KTX `1.47-2-g67253dc`.
...
This **replaces mechanism-as-structural-driver**: all 27 notes share one section structure regardless of kind, instead of the earlier standalone-9 / mutation-6 / variant-4 split. (Section structure lives in [[concept-note-section-structure]] -- revision pending; see Carry-forward below.)
...
- **`bloodfest`** is Solo / PvE. L1 `mode_class` tags it `standalone` while [[mode-vs-mutation-classification]] leaned `mutation`; that disagreement is mechanism metadata and does not affect its experience group.
...
KTX modes are **dynamic**: one server runs them all, and you switch in the console (`/4on4`, `/1on1`, `/ca`, ...), gated by `k_allowed_free_modes`. This is a KTX-era change -- in the old days each mode ran on its own dedicated port (a 1on1 server, a 4on4 server, ...); KTX made it one-server-fits-all. Modes are **match-based**: the command starts a match (countdown, then the timed game).
...
## Carry-forward (methodology reconciliation, not yet done)

Locking this taxonomy obsoletes the kind-driven structure in the sibling docs. Tracked follow-ups:

- [[concept-note-section-structure]] -- collapse the three kind-specific section sets into the one uniform experience-first structure.
- [[concept-note-frontmatter-schema]] -- add `experience_group` and `deathmatch_flag`; demote `kind` to metadata; retire the kind-specific structural layers.
- `~/.claude/skills/game-mode-curate/SKILL.md` -- already carries v2/v3 drift (length bands at line 29; "embrace short" at line 243); reconcile to the uniform structure in the same pass.
- [[triage-rules]] -- unaffected (wiki-harvest scoring is orthogonal to experience grouping).
- **L1 coverage gap** -- the `dmm1`-`dmm5` `ChangeDM` commands (`commands.c:725-729`) are not in `gameplay_mechanics` (only UserModes + mutators were extracted). At least `dmm4` is a played mode. Decide whether to extend L1 extraction to the dmm commands, and audit for any other playable mode-commands the 27 missed.
