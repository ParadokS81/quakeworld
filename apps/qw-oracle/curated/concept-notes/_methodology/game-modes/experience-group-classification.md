...
The `deathmatch-modes` reference note is seeded by the QWiki "Deathmatch Modes" page (dmm0 = coop; dmm1 = items vanish then respawn; dmm2 = weapons stay, consumables vanish with no respawn [Doom2 holdover, unused]; dmm3 = weapons stay, consumables vanish then respawn [most common -- duels, FFA, some TDM]; dmm4 = full-loadout arena, items don't respawn [aim maps amphi/povdmm4, midair]; dmm5-8 = KTPro). **One wiki claim is stale (corrected in the drafted note):** the page lists Deathmatch Modes 5-8 as KTPro-introduced and "absent from KTX." That holds for 6-8 -- KTX clamps `deathmatch` to 0-5 (`world.c:1631`), so there is no `deathmatch 6/7/8` -- but NOT for 5: KTX uses `deathmatch 5` as the clan-arena/wipeout ruleset, live in `items.c:1347/2604`, `client.c:2308`, `bot_items.c:666` and set by `carena_um_init`/`wipeout_um_init`. So the genuinely-absent set is **6-8**; value 5 is present. (No KTPro source is on hand to say whether KTX's arena-5 continues KTPro's Mode 5 or merely reuses the number, so the correction stays grounded in the verifiable KTX side -- do not assert a KTPro gametype-vs-value distinction.)
