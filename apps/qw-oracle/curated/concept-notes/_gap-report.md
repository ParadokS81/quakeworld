...
**Source Layer 3 note:** `apps/qw-oracle/curated/concept-notes/weapon-scripts.md`
...
**Source Layer 3 note:** `apps/qw-oracle/curated/concept-notes/lightning-gun-customization.md`
...
**Source Layer 3 note:** `apps/qw-oracle/curated/concept-notes/player-skins.md`
...
## Generated by: asset-type-curate Round 3 calibration (2026-05-14)

**Source Layer 3 notes:** `apps/qw-oracle/curated/asset-notes/{skybox,hud_element}.md`
**Source investigations:** `apps/qw-oracle/docs/asset-curation/{skybox,hud_element}-investigation.md`
**Upstream targets:** https://ezquake.com/docs/textures (primary); possibly hud + skybox + charset sections share the same root issue
**Upstream staleness:** `textures.md` last edited 2022-11-21 (boundary-stale)

### Framing-level gaps (cross-platform / cross-asset)

#### Case-sensitivity warning for Linux clients and servers

- **Affected asset_types so far:** skybox, hud_element. Expected to surface in more during Phase 3 fan-out (charset, model_texture, map_texture, conback, levelshot, crosshair, etc.). Ultimately applies to every asset_type with a `textures/<subdir>/<name>` install path -- which is most of them.
- **Current upstream coverage:** zero. No ezquake.com page mentions case sensitivity.
- **Source confirmation:** ezQuake's file-lookup layer does not normalize case.
  - HUD elements: `R_LoadPicImage("textures/wad/<name>", ...)` at `r_draw.c:357` -- `<name>` is the lowercase WAD lump name (`anum_0`, `sb_cells`, etc.).
  - Skybox: `R_LoadSkyTexturePixels` probe loop at `r_brushmodel_sky.c:184-209` uses lowercase suffix table `{ "rt", "bk", "lf", "ft", "up", "dn" }`.
  - Charset: `textures/charsets/<name>` and `gfx/conchars.lmp` with lowercase canonical names per `r_draw_charset.c`.
  - Case-mismatched files fail silently -- the engine falls back to stock content with no diagnostic.
- **Affected users:** Linux ezQuake clients (real but minority); **every Linux KTX server admin** (Linux is the dominant deployment surface for public QW servers).
- **Corpus evidence:** community bundles frequently ship uppercase or mixed-case filenames. qw.nu corpus observed: `ANUM_0.png`, `INV2_LIGHTNG.png`, `sb_cells.PNG`, `_Up.tga` / `_Rt.tga` skybox suffix variants. Mixed-case is common, not exceptional.
- **Why it matters:** the user-side failure pattern is "install bundle -> expect HUD/skybox to change -> see nothing changed -> give up." No diagnostic surface. Silent.
- **Suggested upstream content** (~10-15 lines; recommend a shared preamble in `textures.md` or a dedicated "Case sensitivity on Linux" section):
  - State that ezQuake looks up assets by lowercase canonical name.
  - Contrast Windows (NTFS case-insensitive by default) vs Linux (ext4/btrfs/zfs case-sensitive).
  - Recommend renaming on Linux: `for f in *; do mv "$f" "${f,,}"; done` (bash one-liner).
  - Note the warning applies to **every** `textures/` subdirectory: `textures/wad/`, `textures/charsets/`, `textures/<mapname>/`, `env/` skybox faces, model textures, etc.
- **Engine-fix alternative:** OS-level case-normalize at lookup. Out of scope -- slow per-asset-load, filesystem-walk implications, probably rejected upstream. The docs warning is the right fix.
- **PR timing:** wait until Phase 3 fan-out surfaces the pattern in N+ additional slugs to justify a "global warning" framing vs a per-asset-type one-off. Aggregate when ready.

(Future asset-type-curate fan-out slices may append additional framing-level findings here; convention is one section per finding under this Generated-by block, or a new dated Generated-by block per fan-out wave.)
