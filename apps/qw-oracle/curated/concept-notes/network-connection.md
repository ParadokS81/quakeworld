...
Two different antilag implementations exist in the wild -- the original and a fork -- and they behave differently enough that the distinction matters to players; the specifics deserve their own note (operator SME, ParadokS, 2026-06-10; flagged as hedged community knowledge, not source-defensible here). This guide's only claim is that antilag is the server's responsibility and is normally on; see the forward reference in Related.
