...
activation_summary: "Type /ca on a KTX server whose k_allowed_free_modes includes the UM_4ON4 bit (value 8, shared with 4on4 and wipeout). Pre-match only."
wiki_status: hybrid
...
On a KTX server, type `/ca` in the console, before a match has started. The server applies the Clan Arena preset; teams then ready up and the first round begins once both sides are ready.

`ca` shares its activation bit with `4on4` and `wipeout` (`UM_4ON4`), so any server that allows one of the three allows all three. If `k_allowed_free_modes` does not include that bit, the command is refused.

## History

Clan Arena predates its inclusion in KTX. The community long played it on dedicated servers running a standalone Clan Arena modification -- the best known being Clan Arena Champion Edition (Plus) -- and for years joining such a server was the recommended way to play. KTX folded `ca` in as one of its built-in usermodes so any KTX server can host it without a separate mod, alongside its respawn-based variant, wipeout.

## Hosting & settings

Enable the mode by setting its bit in `k_allowed_free_modes`:

```
# server.cfg
// UM_4ON4 bit (value 8) -- also enables 4on4 and wipeout
setadd k_allowed_free_modes 8
```

The Clan Arena ruleset itself is applied by the `/ca` command. The settings that define it:
