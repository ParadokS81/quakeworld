...
- A mirror of every wiki / forum / Discord snippet. Content is earned (see Section 2).
...
3. Capture the decision in Section 7 of this doc with a date and rationale.
...
**superseded** -- the note's entire purpose is covered better by a newer note. Archive the superseded note (see Section 5.3); don't leave two notes that disagree.
...
(These fields are proposed here. Actual addition happens during the first note that needs them; see Section 6 feedback-loop protocol.)
...
- A frontmatter field we need but don't have (example: the `applies_to_versions` proposal in Section 5).
...
Proposed in Section 5 for version-scoped applicability. Not yet added to `README.md` frontmatter because no note has concretely needed it. **Trigger for adoption:** the first note where "when does this apply" cannot be answered cleanly from `related_entities` commit refs alone.

### 2026-04-25 -- `historical` and `superseded` status values

Same as above. Proposed in Section 5; added to `README.md` when the first note enters either state. The `kmap-legacy-keymap-system.md` bootstrap is currently `status: draft` but would be `status: historical` under the proposed schema -- that migration happens when the status-value expansion lands.

### 2026-04-25 -- Tutorial / how-to shape

Proposed in Section 3 as a likely catalog addition when Path 1 imports begin. Not added yet because no imported note exists. **Trigger for adoption:** first Path 1 import whose content doesn't fit the four existing shapes.
...
**Promotion to README rule (rule of three cleared).** Per Section 6 feedback-loop protocol: three instances of the same authoring discipline across two distinct R-mixes (R3+R7 weapon-scripts, R2+R7 lightning-gun + player-skins). Promoted to `README.md` Section  "Voice and length by shape" as an R7-authoring requirement. The four labeled grounds:
...
**Trigger for formalizing as a rule:** a second note intentionally using the same structure, ideally of a different shape (R5 infrastructure + R7 advice). If the pattern holds across 2+ notes, promote to a Layer 3 authoring rule in Section 3 (note shape) or Section 5 (template). Until then, treat as weapon-scripts-specific observation.
...
**Third instance landed 2026-04-25 (late):** `player-skins.md` (Domain walkthrough shape, R2+R7 flavor) opens with Summary + a five-bucket-and-two-paths Mental Model table; everything from Section  "Identification -- file-based path" onward is drill-down depth. Different opener shape (bucket+path table vs three-method-glance vs summary+mechanical-behavior), same structural principle: the first ~30 lines form a complete short answer, the rest is depth. Pattern now holds across 3 notes of two distinct R-mixes (R3+R7 once, R2+R7 twice).

**Promotion to README rule (rule of three cleared).** Per Section 6: three confirming instances across multiple R-mixes is the bar. Promoted to `README.md` Section  "Voice and length by shape" as a structural requirement for notes longer than ~80 lines:
...
**Currently captured:** cshift family cited inline in `player-skins.md` Section  "Powerup-carrier visibility -- dynamic lights and glow"; gap-report candidate `first-person-pov-self-state-indicators` reframed 2026-04-25 to flag this open question.
