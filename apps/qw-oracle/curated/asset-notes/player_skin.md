...
- `../concept-notes/player-skins.md` -- the gameplay walkthrough that uses this asset_type plus adjacent rendering systems, symmetrically for enemy AND teammate configurations (programmatic tinting via `r_enemyskincolor` / `r_teamskincolor` / `r_skincolormode` / `r_fullbrightskins`, powerup-carrier visibility via `r_powerupglow` / `r_dynamic` / `gl_flashblend`, per-player tracking via `enemyforceskins` / `teamforceskins`, corpse readability via `cl_deadbodyfilter` / `cl_gibfilter`, ruleset / FPD gates, recommended recipes). When a player asks "how do I make enemies pop?" or "how do I configure team skins for callouts?", the concept-note is the answer; this asset-note covers the file-loading mechanism the concept-note builds on.
