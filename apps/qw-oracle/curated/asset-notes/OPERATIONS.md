...
> This asset_type's full gameplay story extends beyond file-loading into both enemy-side and team-side configuration: programmatic tinting (`r_enemyskincolor` / `r_teamskincolor` / `r_skincolormode` / `r_fullbrightskins`), powerup-carrier visibility (`r_powerupglow` / `r_dynamic` / `gl_flashblend`), per-player tracking (`enemyforceskins` / `teamforceskins`), corpse readability (`cl_deadbodyfilter` / `cl_gibfilter`), ruleset gates (MTFL fullbrightskins lock), FPD bit 256 (`FPD_NO_FORCE_SKIN`). The asset-note covers the `.pcx`/`.tga`/`.png` file loading mechanism; the partner concept-note covers "how a player configures enemy and teammate visibility for competitive play" -- a vertical slice across multiple L1 anchors. Symmetric coverage matters here for embedding retrieval -- queries about teammate skin configuration must surface the same note as queries about enemy visibility.
