...
This doc is Pass 2 of the 2026-04-22 knowledge-service realignment roadmap. Pass 3 renders the content here into the interactive HTML dashboard.
...
- A sixth field, **verification status**, labels how strongly the AST supports the claim. See "Verification statuses" below.
...
Pass 3's dashboard surfaces these statuses in its detail panels.
...
- `.kmap` - **orphaned_historical**. ezQuake removed keymap loader support in commit `46b5046` (2014-01-12). Files still ship in `ezquake.pk3/keymaps/` but no current C code consumes them. Document rather than delete, so future readers see why `.kmap` files appear in installs.
...
- `.log`, `.loc`, `.lit`, `.xml`, `.dat`, `.spr`, `.qwz` - **audit pending**. These are the seven remaining "missing loader families" added speculatively in commit `119dd0e` (2026-04-22 01:00). The first five are almost certainly ast_verified once the loader-site extractor's classifier walks each family; `.qwz` may be `seed_only_no_ast_support` (external qwdtools decodes it before engine playback). A dedicated follow-up will stamp each entry's status. See HANDOVER: "Pass 2 follow-up: asset-extension per-row verification-status audit."
...
Pass 2 scopes this doc to ezQuake. FTE, MVDSV, and KTX ports (Phase 2d / 2e in the oracle roadmap) inherit the same template. Each port adds its own verification-status audit per type; a type that's ast_verified for ezQuake may start life as `audit_pending` for FTE and graduate as the extractor port matures.
