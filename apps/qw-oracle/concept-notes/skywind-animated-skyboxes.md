...
- **`r_skywind`** (cvar, default `"1"`) - scale factor on the configured animation. It appears in exactly two places: `r_skywind.integer` at `src/r_brushmodel_sky.c:120` gates the sidecar auto-load on skybox change (when non-zero and a skybox loads, `Skywind_Load_f()` runs automatically), and `r_skywind.value` at `src/r_brushmodel_sky.c:523` divides `skywind_period` when computing the animation phase — so higher values shorten the period and speed the motion up. `0` is a degenerate setting: it suppresses auto-load, but if `skywind_dist` is already non-zero from a prior `skywind` command the divide-by-zero in the phase formula is undefined. The help group-id and `type: float` both come from `help_variables.json`.
...
- **Oracle MCP "why doesn't skywind do anything" queries** - three failure modes worth distinguishing. First, `skywind_dist` is `0` - this is the actual enable/disable gate, via `Skywind_Active()`, and `0` is the default until the user runs `skywind <dist ...>` or loads a sidecar cfg. Second, the active skybox has no alpha channel - nothing to blend, so even an active configuration renders a static sky. Third, `r_skywind` is `0` - the auto-load on skybox change is suppressed, so per-map sidecar cfgs do not apply automatically.
