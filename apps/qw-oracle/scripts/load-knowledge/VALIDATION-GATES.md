...
| `description_embedding_stale` | Volatile hint flag: set TRUE by description writers, cleared FALSE by the embed pass. Informational only -- the hash above is the real re-embed signal, so it is stripped here. See SCHEMA.md "Embedding freshness". |
