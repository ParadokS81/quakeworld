#!/usr/bin/env python3
"""Seed-rule verifier for ezQuake asset path rules.

Cross-checks every rule in
`packages/qw-config/seeds/ezquake-asset-path-rules.yaml` against the
current ezQuake source. For each rule with a `source_ref` of the form
`<file>:<line>`, the verifier:
...
Output: <repo>/packages/qw-config/src/data/ezquake-asset-path-rules-verified.json
...
SEED_YAML = Path(_args.seed).resolve() if _args.seed else (HERE / "seeds/ezquake-asset-path-rules.yaml")
OUTPUT_JSON = Path(_args.output).resolve() if _args.output else (HERE / "output/ezquake-asset-path-rules-verified.json")
...
def _enclosing_function_at(tu, target_path: str, line: int) -> Optional[tuple[str, str]]:
    ...
    def visit(node):
        ...
        if (
            ...
            # Check the function's full extent (covers the body).
            ...
            if start <= line <= end:
                ...
                return  # first match wins; outer function is more specific than any enclosing nesting
        for c in node.get_children():
...
def verify_rules(rules: list[dict]) -> list[VerifiedRule]:
    # Parse each unique target file exactly once.
...
def main() -> int:
    print("ezQuake asset path-rules seed verification")
    ...
    if not isinstance(seed, dict) or "path_rules" not in seed:
        print(f"ERROR: seed does not contain a path_rules list", file=sys.stderr)
        return 1
