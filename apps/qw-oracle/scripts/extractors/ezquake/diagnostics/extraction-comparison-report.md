...
| **tree-sitter-c** (Node + `tree-sitter@0.25`) | Originally planned for trailing-comment extraction | No. Segfaults on WSL/Node 20 with the native binding. Pivoted to textual pass (above). `py-tree-sitter` (Python bindings) has stable binary wheels and is the likely pick if tree-sitter does become relevant. (Historical note: this report's original wording speculated tree-sitter would be the right toolchain for future KTX call-site extraction. That projection was superseded by D2 of the 2026-05-04 KTX onboarding arc -- canonical KTX is pure C and uses libclang like the other four engines; tree-sitter is reserved for the dusty-ktx fork's `qcsrc/` QuakeC tree.) |

**Net:** libclang does more than anticipated. The tree-sitter complementary pass was not needed for ezQuake. (The original report also predicted tree-sitter would be the right pick for KTX later; that prediction was superseded by D2 of the 2026-05-04 KTX onboarding arc -- canonical KTX uses libclang. See `apps/qw-oracle/scripts/extractors/EXTRACTOR-PLAYBOOK.md` for the post-KTX-onboarding pattern catalog.)
