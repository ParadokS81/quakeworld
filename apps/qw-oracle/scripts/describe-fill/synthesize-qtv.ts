...
// Mirror of synthesize-mvdsv.ts -- engine-swapped for QTV.
//
// Phase-3 QTV describe-fill PERSISTENCE driver.
//
// The QTV mirror of synthesize-ktx.ts's persistence half. The synthesis
...
// records to, and the in-scope idempotency fingerprint. QTV is a frozen
// vendored Go snapshot (anchor version 1.16-dev). Built small on the
// first subsystem batch and reused by every later QTV batch.
//
...
//                     QTV entity rows (fill-not-create -- D9). Idempotent:
...
//                     the in-scope QTV set (project='qtv',
//                     type IN cvar/command) for the idempotency proof.
//
//   --status          Read-only. How many in-scope QTV entities carry a
...
// SQL text, which cannot bind a JSONB JS value). QTV's persist must bind
...
// ---------------------------------------------------------------------------
// In-scope QTV configurable buckets (C1 denominator shape; D1)
// ---------------------------------------------------------------------------

// WHY a shared constant: the fingerprint scope, the status scope, and any
// future coverage query must use the SAME buckets or the idempotency
// proof and the coverage gate silently disagree.
// QTV has ONLY cvars + commands (0 cmdline_param, 0 info_key, confirmed
// live in Phase 2).
const IN_SCOPE_TYPES = ['cvar', 'command'] as const;
...
async function computeFingerprint(
  ...
  const fp = rows[0]?.fp;
  if (!fp) {
    throw new Error('computeFingerprint: md5 returned null -- no QTV entities in scope');
  }
  return fp;
}
...
async function applyRecords(
  ...
  await sql.begin(async (tx) => {
    ...
    // --- 5. Summary ---
    process.stdout.write('\n=== synthesize-qtv persist summary ===\n');
    ...
    process.stdout.write(`in-scope QTV fingerprint (${dryRun ? 'rolled-back' : 'committed'}): ${fingerprint}\n\n`);
...
async function fingerprintCmd(): Promise<void> {
  ...
  process.stdout.write('=== --fingerprint: QTV committed-row md5 ===\n');
  process.stdout.write(`scope: project='qtv', type IN (${IN_SCOPE_TYPES.join(',')})\n`);
...
async function statusReport(): Promise<void> {
  const rows = await sql<Array<{ entity_type: string; evaluated: number; remaining: number }>>`
    ...
    FROM entities
    WHERE project = 'qtv' AND type IN ${sql(IN_SCOPE_TYPES as unknown as string[])}
    GROUP BY type
    ORDER BY type
  `;
  ...
  process.stdout.write('=== --status: QTV describe-fill progress ===\n\n');
  process.stdout.write(`In-scope QTV entities: ${totalEval + totalRem}\n`);
  ...
  try {
    if (args.includes('--persist')) {
      ...
      if (!recordsPath || recordsPath.startsWith('--')) {
        process.stderr.write('synthesize-qtv --persist: missing records file.\nUsage: --persist <records.json> [--dry-run] [--operator-override <name,name>]\n');
        process.exit(1);
      }
      ...
      if (!glob || glob.startsWith('--')) {
        process.stderr.write('synthesize-qtv --from-ledger: missing glob.\nUsage: --from-ledger <glob> [--dry-run] [--operator-override <names>]\n');
        process.exit(1);
      }
      ...
      process.stderr.write(
        'synthesize-qtv: no mode specified.\n' +
...
if (import.meta.main) {
  main().catch((err: unknown) => {
    process.stderr.write(`synthesize-qtv error: ${err instanceof Error ? err.message : String(err)}\n`);
    process.exit(1);
  });
}
