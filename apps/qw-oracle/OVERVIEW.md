...
**Tags loaded:** 3.6.1, 3.6.2, 3.6.5, 3.6.6, 3.6.8, 3.6.9, plus `head`. All 7 re-extracted 2026-04-25 against the current unified + legacy extractors after a load-version stale-row cleanup landed (commit `454fe4a`); pre-existing data-currency gaps (smackdrive @ 3.6.1, fpd_enable_player_count @ 3.6.1, 18 cvar citation gaps) all cleared. Earlier ezQuake tags (3.0 → 3.6.0) are roadmapped in `docs/layer1-extraction-roadmap.md`. Layer 1 quality gates run via `quality-grid` CLI subcommand; current state: 5/5 regression PASS, 4/6 anomaly CLEAN, 1 informational, 1 schema-gap residue (per-version source_state on 2 entities, see HANDOVER).
...
- **Phase 2f historical backfill proper** - walk every ezQuake tag (~15 total), run full extractor sweep per tag, diff consecutive pairs, enrich with PRs. All architectural prerequisites shipped through Batch 3 + extraction-review skill/CLI + Workstream A tweaks (2026-04-24). **Sanity-sample calibration cleared 2026-04-24** per `docs/superpowers/specs/2026-04-24-extraction-review-sanity-sample-calibration.md` — thresholds hold at §8 starting values; one detector bug (commit-UNKNOWN sentinel in `clusters.ts`) fixed in-flight; one semantic-pass abbreviation gap captured to HANDOVER. Phase 2f is unblocked but **deprioritized 2026-04-25** in favor of Layer 3 foundation work via `guide-rewrite`. Will resume when Layer 3 coverage stabilizes or when slipgate-app surfaces a history-dependent need. HANDOVER entry: `Phase 2d-2h: remaining QW knowledge rollout`.
