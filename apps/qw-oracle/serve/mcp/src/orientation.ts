...
- Layer 1 (engine + game-content facts): cvars, commands, macros, command-line params, rulesets, info_keys, log_templates, protocol_messages, qc_builtins, cvar_aliases, match_events, maps, gameplay mechanics, game modes. Use lookup_entity / search_entities (entity types: cvar, command, macro, cmdline_param, ruleset are the default set searched when type is omitted; pass type explicitly for match_event, info_key, log_template, protocol_message, qc_builtin, or cvar_alias -- eleven types total, all case-insensitive by name) / lookup_map / search_maps / lookup_mechanic / search_mechanics / lookup_gameplay_entity / search_gameplay_entities for definitive engine facts. For a whole KTX game mode assembled in one envelope (catalog + enforced settings + the entity/mechanic override rows KTX swaps in for that mode, e.g. bloodfest's monster roster or ca's fall-damage suppression + activation cvars + concept note), use describe_mode, e.g. describe_mode('ca'). Engine projects covered: ezquake, fte, mvdsv, qwcl, ktx, plus the server-side tools qtv (the Go QTV stream proxy) and qwfwd (the C UDP forwarder/proxy) -- both queryable via lookup_entity / search_entities with project='qtv' or project='qwfwd'. Gameplay rows carry a gameplay_source: omit it to search all sources (id1 base Quake + ktx), or pass 'ktx'/'id1' to scope. KTX gameplay content uses kind discriminators on search_mechanics: 'game_mode' (mode catalog: 1on1 / ca / wipeout / race / bloodfest / lgc / ...), 'mode_default' (per-cvar settings each mode applies -- filter to one mode with the mode parameter, e.g. mode='ca'), 'constant', 'env_hazard', 'powerup_behavior', 'election_type', 'death_rule', 'score_system', 'drop_item', 'loc_macro', 'teamplay_message' (constant / env_hazard / powerup_behavior / death_rule are kind names KTX shares with base-game id1 rows -- pass gameplay_source='ktx' to scope to KTX's own rows in those kinds); search_gameplay_entities admits item / weapon / projectile / monster kinds (monster = the KTX bloodfest roster). KTX match-event types (death / damage / pick_mapitem / pick_backpack / drop_backpack / pick_powerup / drop_powerup) are entities.type='match_event', reachable via lookup_entity / search_entities with type='match_event'.
- Layer 2 (chat history): use search_solved_issues for "has this been debugged before" questions. Returns topic-coherent chat threads (reconstructed from the Discord corpus) for citation. Discord-only; pre-2016 IRC content is not in this corpus.
