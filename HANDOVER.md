...
- **KTX Layer 1 Onboarding** -- planning COMPLETE; all 9 phase MDs approved; orchestrator handoff written at `docs/superpowers/parking/2026-05-05-ktx-onboarding-orchestrator-handoff.md`. Next is opening a fresh terminal with arc-orchestrator skill. Plan: `docs/superpowers/plans/2026-05-04-ktx-onboarding/README.md` (9 phases total).
...
- **qw-oracle slim-doc Arc 1 refresh sweep** — `apps/qw-oracle/{README,SCHEMA,OVERVIEW}.md` reference deleted SQLite artifacts (`data/knowledge.db`, `data/qw.db`, `scripts/load-knowledge/schema.ts`, `SCHEMA_V*_ADDITIONS_SQL` / `applySchema` pattern, `better-sqlite3` as the L1/L2 store) + stale tool counts ("Ten tools" → 12 post-Phase-6) + stale processing claims ("Layer 2 processing pipeline incomplete" → Phase 3 shipped) + stale runtime ("Node 20+ / Bun" → Bun-only per D2) + stale Schema-version framing ("Schema v18" in CLAUDE.md Status line). Drift now spans Arc 1 Phases 2-6, not just Phase 2. CLAUDE.md tech stack + always-on rules drained inline 2026-05-02 (post-Phase-4 cleanup); the broader sweep is one focused pass to align with Postgres + `db/migrations/` reality + post-Phase-6 MCP shape + Layer 3 first-class status. Touches all three slim docs plus possibly `scripts/load-knowledge/CLAUDE.md` and the `**Status:**` line in app `CLAUDE.md`. **Trigger: KTX onboarding arc end-of-arc.** Sequenced AFTER the KTX migration lands (`008_ktx_log_template_logfile_channel.sql`); doing the sweep before would document 4 `log_template_versions.channel` values and re-stale immediately. See KTX onboarding spec "Doctrine fixes deferred to end-of-arc" block.
