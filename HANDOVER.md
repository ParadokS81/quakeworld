...
- **matchscheduler doc system reconciliation** — context/ predates monorepo doctrine and has grown: 119 of 137 `.md` under apps/matchscheduler are unindexed from its CLAUDE/OVERVIEW/README (89 in context/, 28 in docs/) per the 2026-09-11 wrap's orphan count — trigger: FIRED 2026-09-11 (sign-in fix session was the "next matchscheduler friction"); `[decision]` run it, or re-park as operator-initiated given the app is Maintenance and slated for rebuild inside slipgate web — docs/superpowers/plans/2026-04-29-docs-redesign-session-a.md.
...
- `[agent-food]` **Post-migration doc sweep (WSL-era claims)** — root CLAUDE.md WSL section confirmed still present 2026-08-11; qw-oracle DEVELOPMENT.md npm/localhost claim not re-confirmed this pass. New evidence 2026-09-11: matchscheduler hosting + functions deploys ran end-to-end from the cockpit (Firebase CLI authenticated there), so the "all projects in WSL" framing is stale for that app too; the operator's desktop is now Omarchy, not Windows/WSL.
- `[agent-food]` **matchscheduler OVERVIEW.md refresh** — last touched 2026-04 (5e0e3538); landed since: Big4 manual sync (scheduled poll retired 2026-09-11), /ktx-documentation.html, Discord-shareable squad join links, sign-in popup guard for tiling window managers — apps/matchscheduler/OVERVIEW.md.
- `[agent-food]` **Big4 poll revival, only if The Big 4 returns** — re-export `scheduledBig4Sync` in apps/matchscheduler/functions/index.js + deploy; `BIG4_API_KEY` is in no environment anymore (the kept manual admin sync needs it too) — provision via Secret Manager in the same shape as `DISCORD_CLIENT_SECRET` (apps/matchscheduler/DEPLOYMENT.md).
