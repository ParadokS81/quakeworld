...
- **Slipgate config-viewer doc_only filtering investigation** — slipgate-app currently ships 184 doc_only entities (139 cvars + 41 commands + 2 cmdline + 2 macros) into snapshots; zero source_state references in slipgate's TS/Svelte/Rust code, so config viewer likely surfaces zombies to users. Two questions: (a) is slipgate consuming qw-oracle's `build-snapshot` outputs at all, or still on old self-scraped JSON? (b) should `build-snapshot.ts:80` filter `source_state != 'doc_only'` at producer side, or should slipgate filter at consumer side? See fresh-session prompt at `docs/superpowers/parking/2026-05-01-slipgate-snapshot-consumption-and-doc_only-filtering.md`.
