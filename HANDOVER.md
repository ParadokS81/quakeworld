...
- [Layer 2 thread reconstruction](docs/superpowers/parking/2026-05-03-layer2-thread-reconstruction.md) -- LLM-driven re-segmentation of Discord chat into topic threads (replacing timestamp-bucket sessions as the primary retrieval unit). Architecture + research summary parked 2026-05-03. Cost equivalent to the original per-session summarize plan that was already deferred to Arc 3. Trigger: post-Phase-8 deploy when `query_log` evidence shows whether FTS-only Layer 2 retrieval is the bottleneck.
