...
- **qw-oracle slim-doc Phase 2 refresh sweep** — `apps/qw-oracle/{README,SCHEMA,OVERVIEW}.md` still reference deleted SQLite artifacts (`data/knowledge.db`, `scripts/load-knowledge/schema.ts`, the `SCHEMA_V*_ADDITIONS_SQL` / `applySchema` migration pattern, `better-sqlite3` as the Layer 1 store). CLAUDE.md tech stack + always-on rules drained inline 2026-05-02; the broader sweep is one focused pass to align with Postgres + `db/migrations/` reality. Touches all three slim docs plus possibly `scripts/load-knowledge/CLAUDE.md`.
