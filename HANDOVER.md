...
- [Phase 2d-2h: remaining QW knowledge rollout](#phase-2d-2h-remaining-qw-knowledge-rollout) — **QWCL 2.33 SHIPPED 2026-04-25** + **FTE Phase 2d-core SHIPPED 2026-04-26** + **FTE Phase 2d-bundle SHIPPED 2026-04-27** (28 asset_category + 61 extensions + 13 path_rules + 25 cvar_bindings + 717 loader_sites; quality-grid 30/30; 3 Path-1 fixtures green) + **Game-mechanics arc 1 SHIPPED 2026-04-27 evening** (schema v14: gameplay_sources/gameplay_entity_defs/gameplay_mechanics; 37 entities + 41 mechanics from id1 QC; 4 new MCP tools; SERVER_VERSION centralized 12 sites→1; v4 splits: telefrag/exit_level_kill + trigger_hurt env_hazard; commits a3dddc6→6110901) + **Phase 2e MVDSV mid-flight 2026-04-27 late evening** (spec `5077a1b` + plan `4252a08` + Task 1 pass-1 inventory `096a7f3` + plan refinement `f3625a9`; 25 tasks remain — see "Phase 2e MVDSV implementation in progress" entry below). ezQuake deep-time walk at v3.0 floor. Remaining: Phase 2e MVDSV (Tasks 2-26), Phase 2e-deep-time MVDSV (after KTX), Phase 2e KTX cvars (after MVDSV), Phase 2e KTX gameplay overrides, qw_event_log validation harness, Phase 2g MCP tool upgrades, Phase 2h automation.
- [Phase 2e MVDSV implementation in progress](#phase-2e-mvdsv-implementation-in-progress) — **NEW 2026-04-27 late evening.** Brainstorming + spec + plan + Task 1 (pass-1 inventory) shipped this session. 25 tasks remain (Schema v15 migration, three-variant clang_config, seven engine-private handlers, four loader adapters, runtime-validation harness against Ciscon's `1.20-dev` dump, 20-row field-accuracy audit, quality-grid probes, extract-tag, doc/memory updates, push). Resumes in fresh terminal via `superpowers:subagent-driven-development`. All context self-contained in spec + plan + notes-pass-1.md.
...
**Status:** ezQuake head + deep-time walk complete (v3.0 to head); QWCL 2.33 shipped; FTE Phase 2d-core + 2d-bundle fully shipped. Next: Phase 2e MVDSV+KTX.
...
## Phase 2e MVDSV implementation in progress

**Added:** 2026-04-27 late evening. **Status:** Plan executing — Task 1 of 26 shipped, 25 remain. **Verification first:** `git log --oneline 5077a1b..f3625a9` shows the four commits this session shipped. `cat apps/qw-oracle/scripts/extractors/mvdsv/notes-pass-1.md | head -40` shows the verified pass-1 inventory. `ls research/repos/mvdsv/src/qwprot/src/protocol.h` should exist (qwprot submodule was initialized this session).

### What shipped this session

- **Design spec** at `docs/superpowers/specs/2026-04-27-mvdsv-extraction-design.md` (commit `5077a1b`, 716 lines). Captures all five Q1-Q5 brainstorming decisions, the seven entity types, the variant model (server-base + Win + Linux with FTE_PEXT/MVD_PEXT extensions ON, NQPROGS OFF), schema v15 migration shape, and runtime-validation strategy against Ciscon's `1.20-dev` dump.
- **Implementation plan** at `docs/superpowers/plans/2026-04-27-mvdsv-layer1-extraction.md` (commit `4252a08` initial, refined `f3625a9`, 3500+ lines). 26 tasks across 5 phases (Foundation, Per-handler, Loader, Validation, Ship). Bite-sized steps with full code blocks, exact file paths, exact commands, expected output.
- **Task 1 ship**: pass-1 inventory at `apps/qw-oracle/scripts/extractors/mvdsv/notes-pass-1.md` (commit `096a7f3`). Concrete counts: `Cvar_Register` 194 sites, `Cmd_AddCommand` 114 sites, `Con_Printf` 717 sites, `SV_BroadcastPrintf` 35 sites, `Info_Get` 62 sites (MVDSV-local wrapper), `std_builtins` 83 entries + `ext_builtins` 24 sparse pairs.
- **qwprot submodule initialization** at `research/repos/mvdsv/src/qwprot/` (commit `07a72852` per upstream). Provides `protocol.h` with `svc_*`/`clc_*`/`FTE_PEXT_*`/`MVD_PEXT1_*`/`PROTOCOL_VERSION_FTE*` definitions. Without this, clang can't resolve protocol-extension symbols in MVDSV source.
- **Plan refinement** absorbing 5 surprises Task 1 surfaced (`f3625a9`): Task 4 clang_config drops speculative -D flag list in favor of CMakeLists-verified defines + `-I src/qwprot/src` for protocol.h bit-shift values; Task 10 info_keys API map expanded with `Info_Get` and four other Info_* operations; Task 11 log_templates drops nonexistent `SV_BroadcastTPrintf`/`SV_ClientTPrintf`, adds `SV_BroadcastPrintfEx` and `SV_BroadcastCommand`; Task 12 qc_builtins targets `std_builtins` + `ext_builtins` (the actual table names); Task 6 cvars confirmed CVAR_ARCHIVE absent.

### What remains (Tasks 2-26)

**Phase A foundation (Tasks 2-5):** Schema v15 migration (TS code in `schema.ts` + `types.ts`), SCHEMA.md update, clang_config three variants, driver skeleton.

**Phase B handlers (Tasks 6-13):** Seven engine-private `_handler_*.py` files at `apps/qw-oracle/scripts/extractors/mvdsv/`, each with detection + sanity-check + commit, plus full smoke pass.

**Phase C loaders (Tasks 14-17):** Four TypeScript adapters wired into `ADAPTERS` dict in `load-version.ts` (with the `Partial` workaround in Task 2 step 10 reverted in Task 17 step 3 once all four are present).

**Phase D validation (Tasks 18-23):** Load all seven types into knowledge.db, stage Ciscon's dump fixture (`apps/qw-oracle/scripts/extractors/mvdsv/validation-fixtures/ciscon-1.20-dev-2026-04-27.log` — already at `/mnt/c/Users/Administrator/Downloads/qw-1.log`, just needs copying), build `diff-runtime.sh` harness, iterate handler fixes until residual runtime-validation gap is fully categorized, 20-row field-accuracy audit, MVDSV regression + anomaly probes added to `quality-grid.ts`.

**Phase E ship (Tasks 24-26):** `extract-tag` for atomic provenance (records mvdsv head commit SHA in `versions` table), optional `build-snapshot --project mvdsv`, doc updates (`HANDOVER.md` close out this entry, `apps/qw-oracle/CLAUDE.md` status paragraph, `EXTRACTOR-PLAYBOOK.md` per-engine table + any new patterns), memory file update at `~/.claude/projects/-home-paradoks-projects-quakeworld/memory/`, `git push origin main`.

### Resumption

Fresh terminal, paste:

> Continue Phase 2e MVDSV implementation at Task 2. Read the plan at `docs/superpowers/plans/2026-04-27-mvdsv-layer1-extraction.md`, the spec at `docs/superpowers/specs/2026-04-27-mvdsv-extraction-design.md`, and the pass-1 inventory at `apps/qw-oracle/scripts/extractors/mvdsv/notes-pass-1.md`. Use `superpowers:subagent-driven-development` to dispatch implementer + spec reviewer + code quality reviewer subagents per task. Tasks 1 of 26 is done; Tasks 2-26 remain. Working dir is the main tree at `/home/paradoks/projects/quakeworld` on branch `main`; commit directly to main per the operator's git workflow.

### Pressure

Active. Live development arc — one of two open knowledge-foundation tracks (the other being Phase 3.5a/3.5b slipgate IA work). Resume in next dedicated session.
