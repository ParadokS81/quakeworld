...
- [Layer 1 cvar_versions vs entities.first_seen_version drift](#layer-1-cvar_versions-vs-entities-first_seen_version-drift) — **Surfaced 2026-04-25** during lightning-gun-customization authoring. `entities.first_seen_version` reports `head` for `gl_custom_lg_color`, `gl_custom_lg_fullbright`, `gl_custom_lgpack_color`, `cl_truelightning` even though `cvar_versions` has rows for them back to 3.6.1 across every tag. Authoritative answer is in `cvar_versions`; the entity-level summary field is stale or computed wrong. Not a Layer 3 blocker (workaround: query `cvar_versions` directly). Affects how Layer 1 reports "when did this entity first appear" — relevant to active-assistance answers and any consumer that queries the entity row alone.
...
## Layer 1 cvar_versions vs entities first_seen_version drift

**Added:** 2026-04-25 during `lightning-gun-customization.md` authoring.
**Status:** Open. Not blocking Layer 3 work — workaround is querying `cvar_versions` directly. Affects accuracy of any consumer that reads `entities.first_seen_version` alone.
**Verification first:** `sqlite3 apps/qw-oracle/data/knowledge.db "SELECT e.name, e.first_seen_version, MIN(cv.version) AS earliest_cvar_version FROM entities e JOIN cvar_versions cv ON cv.entity_id = e.id WHERE e.project='ezquake' AND e.name IN ('gl_custom_lg_color', 'gl_custom_lg_fullbright', 'gl_custom_lgpack_color', 'cl_truelightning') GROUP BY e.name;"` — if the two columns match for all four rows, this entry has been resolved.

### What surfaces

The `entities.first_seen_version` column reports `head` for entities whose `cvar_versions` history shows rows back to 3.6.1 across every tag:

- `gl_custom_lg_color` — entities says `head`, cvar_versions has 3.6.1, 3.6.2, 3.6.5, 3.6.6, 3.6.8, 3.6.9, head
- `gl_custom_lg_fullbright` — same pattern
- `gl_custom_lgpack_color` — same pattern
- `cl_truelightning` — same pattern (legacy alias; possibly different cause)

`cl_truelightning` may be explainable: `host.c:580` registers it via `Cmd_AddLegacyCommand` mapping to `cl_fakeshaft`. If the legacy-alias form is what `first_seen_version='head'` reflects, that may be intentional — the alias-via-`Cmd_AddLegacyCommand` was added in head, but the underlying cvar (`cl_fakeshaft`) goes back further. The `gl_custom_lg_*` family doesn't have this excuse — they're plain cvar declarations at `r_aliasmodel.c:93-94, 138`.

### Likely causes

1. `entities.first_seen_version` is computed wrong during loader pass (e.g., looks at the latest cvar_versions row instead of the earliest, or only considers rows whose source_file was added at a specific point).
2. Loader-side bug where the entity row gets re-inserted at head while existing cvar_versions rows survive a name match.
3. Schema-evolution artifact — `entities.first_seen_version` was added later and never backfilled correctly.

Without digging in, can't tell which. The pattern (4 entities all incorrectly first_seen=head, all in `r_aliasmodel.c` for the gl_custom_lg* family + `host.c:580` for cl_truelightning) suggests something systematic rather than per-entity drift.

### Impact

- **Layer 3 notes:** when authoring claims "post-3.6.6 addition" or "head-only" based on `entities.first_seen_version`, the claim is wrong if cvar_versions disagrees. Fix in lightning-gun-customization.md (corrected 2026-04-25); future notes should query cvar_versions directly.
- **MCP tools:** `lookup_entity` and `search_entities` likely surface `entities.first_seen_version` for the "when added" answer. Wrong for these four.
- **Active-assistance answers:** "this cvar was added in version X" answers will be wrong for affected entities.

### Recovery

Investigate during the next extractor / loader session. Likely a loader-side fix (recompute `entities.first_seen_version` from the joined cvar_versions data). Run a sweep query first to find all entities with the same drift pattern — there may be more than four.

```bash
sqlite3 apps/qw-oracle/data/knowledge.db "SELECT e.name, e.first_seen_version AS entity_says, MIN(cv.version) AS versions_says FROM entities e JOIN cvar_versions cv ON cv.entity_id = e.id WHERE e.project='ezquake' GROUP BY e.id HAVING entity_says != versions_says LIMIT 50;"
```

### Pressure

Low. Not a Layer 3 blocker. Worth fixing before Phase 2g MCP tool upgrades land, because those tools will surface the wrong answer to users.

### Related

- Layer 3 note that surfaced this: `apps/qw-oracle/concept-notes/lightning-gun-customization.md`
- Loader: `apps/qw-oracle/scripts/load-knowledge/load-version.ts`
- Schema: `apps/qw-oracle/scripts/load-knowledge/schema.ts` (the `entities` table definition with `first_seen_version`)
