...
- **Asset-type curate skill arc — Phase 3 fan-out** — [parking](docs/superpowers/parking/2026-05-13-asset-type-curate-skill-arc.md) — paused, waiting on operator session to run queued fan-out waves.
...
- **qwiki-v1-beta Phases 5-8 (Modes vertical slice)** — unstarted, deferred until picked up — trigger: operator-initiated — docs/superpowers/plans/2026-05-12-qwiki-v1-beta/README.md.
...
- **QTV/QWFWD concept-note authoring** — author master-server heartbeat + MVD-streaming/parse_delay notes (D9), defer qtv_password — trigger: pending operator greenlight — docs/superpowers/parking/2026-05-31-qtv-qwfwd-documentation.md.
...
- **MCP tool upgrades** (version param / get_entity_history / date filters) — deprioritized, verified absent 2026-05-21 — trigger: operator-initiated on version-aware demand — apps/qw-oracle/docs/arc-history.md (2026-05-21 tier record).
...
- **Slipgate SCHEMA.md for snapshot consumer interface** — trigger: next slipgate UI arc surfacing version-arc badges — docs/superpowers/parking/2026-04-26-slipgate-schemamd.md.
...
- `[agent-food]` **mvdsv L1 extraction gap: `ucmds[]` table not walked** — `dl`/`dlist`/`demolist` absent from L1; extend extractor (research/repos/mvdsv/src/sv_user.c:3325-3340).
...
- `[agent-food]` **L3 concept note: QW team-chat visibility across the stack** — cross-codebase synthesis (KTX/MVDSV/ezQuake/fteqtv), ~30-45 min — apps/qw-oracle/curated/concept-notes/qw-team-chat-visibility.md (not yet authored).
...
- `[agent-food]` **rulesets.c PR #1132 `-ruleset` description follow-up** — still lists 3 of 6 working values — docs/superpowers/parking/2026-05-26-handoff-ezquake-bugfix-prs-rulesets-sys_posix-investigate.md.
- `[agent-food]` **sys_posix.c PR #1133 `-nostdout` description follow-up** — generic pre-fix wording needs sharpening — docs/superpowers/parking/2026-05-26-handoff-ezquake-bugfix-prs-rulesets-sys_posix-investigate.md.
...
- `[agent-food]` **Cross-project audit cadence OVERDUE** — new projects/entity-types/migrations fired since 2026-05-06 — apps/qw-oracle/scripts/extractors/EXTRACTOR-PLAYBOOK.md "Cross-project audit cadence" section.
...
- `[agent-food]` **`cl_voip_demorecord` is dead code** — zero read sites in source, flagged in PR #1120 body; slime's call.
- `[agent-food]` **`cl_pext_serversideweapon` OnChange handler missing** — forward-declared, definition absent from tree; slime's call.
...
- `[decision]` **QWiki sandbox modernization arc (spawned 2026-05-09)** — likely superseded by shipped qwiki-v1-beta — operator to confirm-and-delete or revive. apps/qwiki-sandbox/ scaffold confirmed still present 2026-08-11.
- `[decision]` **Phase B drain brainstorm (paused mid-session 2026-05-09)** — sister-arc dependency to the QWiki sandbox modernization row above — operator to confirm-and-delete or revive (same coupling).
...
- `[decision]` D5 doctrine-delivery audit (chunk 6 P2, 2026-08-11): DOCTRINE.md has no pointer, rendering, or mention anywhere in this repo's loaded surface — CLAUDE.md's doc index gained the DOC-PHILOSOPHY.md row this phase (D10) but not one for DOCTRINE.md itself; no skill under `.claude/skills/` (38 files checked) references it; the only case-insensitive "doctrine" hits in project memory (10 lines / 3 files) predate chunk 3 and use the word in the design-spec sense (doc-philosophy / extraction-playbook doctrine), not the DOCTRINE.md artifact. Per W14/W18 (skills render doctrine, never own it): should CLAUDE.md's Documentation index gain a DOCTRINE.md pointer row beside DOC-PHILOSOPHY.md's, and/or should any always-loaded skill prose render a slice? Deliberately unresolved — D5 records, does not repair.
...
- **Lift nginx resolver + variable-upstream pattern to qwiki/qw-stats/phoenix fronts** — unblocks: none (general infra hygiene) — pattern shipped at commit 8f1dfd82 (qw-oracle nginx); audit + lift ~5 min each remaining front.
- **Plugin v-table asset detection (loader-sites handler)** — unblocks: none (engine-agnostic finding, no body to migrate) — only plugin:ezhud affected, low pressure.
- **Cvar-binding handler indirection gap (snprintf chains + CVARFC callbacks)** — unblocks: none (engine-agnostic limitation) — bundle reconciliation treats as seedRetained, AST-corroboration signal partial.
