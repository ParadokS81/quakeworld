...
- [Phase 2d-2h: remaining QW knowledge rollout](#phase-2d-2h-remaining-qw-knowledge-rollout) — ezQuake deep-time walk reached **v3.0 floor (14 versions: v3.0 through head)** 2026-04-25 late; pre-3.0 era de-scoped on community-security framing. Walk infrastructure shipped same session: `extract-tag --skip-prune` + `prune-cross-type-orphans` finalize CLI + per-version `backfill_match` detection. Reusable for FTE/MVDSV/KTX. **QWCL 2.33 SHIPPED 2026-04-25** (first cross-codebase port; 186 cvar / 120 command / 58 cmdline_param at qwcl@2.33; schema v10 widened project CHECK; quality-grid 5/5 F1 PASS). **FTE Phase 2d-core SHIPPED 2026-04-26** (build-6698 SHA 35843773: 2379 cvars / 556 commands / 67 macros / 103 cmdline_params; schema v11 source_root; ezhud plugin in scope; Pass 1 runtime diff deferred). Remaining: Phase 2d-bundle (asset extraction), Phase 2e MVDSV+KTX, Phase 2g MCP tool upgrades, Phase 2h automation.
...
**Status:** ezQuake head + deep-time walk complete (v3.0 to head); QWCL 2.33 shipped; FTE Phase 2d-core shipped. Next: Phase 2d-bundle (asset extraction), then Phase 2e MVDSV+KTX.
...
**Tier 2 — Phase 2d FTE cvars -- SHIPPED 2026-04-26.** See Updated notes above. Remaining FTE work: Phase 2d-bundle (asset extraction) + Pass 1 runtime cvarlist diff (deferred, needs operator FTE boot).
