...
**Sections** -- apply the **per-section contract** in `concept-note-section-structure.md` directly (what each section holds / must NOT hold / how it's formatted). Do not draft from a section list memorized here -- the contract doc is the authority and this skill's summary can lag it. Canonical order:
...
- **Drafter-honesty in commit-body verifications** -- if your commit message body lists "Source verifications: ..." then every entry must be a real verification you performed (read the file, found the value, copied the cite). A blanket "all mechanical claims verified" line has hidden a fabricated number before (a wipeout spawn loadout written as 100/100 HP/armor; source has 100 HP / 200 RA / 80% absorb). The commit body is auditor-readable; operator/orchestrator may spot-check.
...
- Mode kind / experience-group unclear: trust [[experience-group-classification]]; a high-confidence contrary source signal is a HALT (rule 2), never a silent override
